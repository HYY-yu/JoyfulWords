export const zh = {
    taskGallery: {
        "title": "任务画廊",
        "currentStyle": "当前设计风格",
        "noDesign": "未设置",
        "designFailed": "风格加载失败",
        "empty": "暂无已完成的插图或图表",
        "loadFailed": "画廊加载失败",
        "loadMore": "加载更多",
        "preview": "查看大图",
        "previewImage": "查看 {title} · 第 {index} 张",
        "exportFailed": "导出失败，请重试"
    },

    illustration: {
        progress: {"stages": {"pending": "排队中", "analyzing": "分析内容", "submitting": "提交生图", "processing": "生成中", "succeeded": "已完成", "failed": "生成失败", "empty": "无适合内容"}, "billing": {"pending": "待结算", "reported": "已结算", "failed": "结算待处理", "skipped": "不扣积分"}, "stage": "进度", "credits": "{count} 积分", "openArticle": "打开文章", "failed": "生成失败，可返回文章重新生成。"},
        preferenceHint: "应用于所有文章后续生成的图片。",
        applyStyle: "应用风格",
        chooseStyle: "请在文章列表顶部选择设计风格。",
        saveMaterial: "加入素材库",
        materialSaved: "已加入素材库",
        materialFailed: "保存失败，请重试。",
        savedContent: "使用已保存的文章内容。",

        artwork: {
            subjects: "画面主体：",
            analysisFailed: "场景分析未能完成，未提交生图。可重新分析。",
            "selectionHint": "根据选段生成一张无文字配图。",
            "priceHint": "每张 {price} 积分，最多 {total} 积分；按成功张数计费。",
            "analyzing": "正在分析配图场景…",
            "progress": "已完成 {completed} / {total} 张，失败 {failed} 张",
            "partial": "已完成 {completed} 张，失败 {failed} 张；已完成图片可下载。",
            "noCards": "当前内容未找到适合的配图场景，未生成或扣取图片积分。",
            "cardEvidence": "场景来源与理由",
            "enlarge": "放大查看",

            "source": "内容来源",
            "sources": {
                "article": "整篇文章",
                "selection": "选段"
            },
            "sourceText": "选中或粘贴真实内容（最多 20,000 字）",
            "orientation": "画面方向",
            "orientations": {
                "square": "方形",
                "landscape": "横向",
                "portrait": "纵向"
            },
            "language": "语言",
            "generate": "生成配图",
            "credits": "积分",
            "hint": "配图以无文字场景延续文章视觉 IP；不会生成信息卡片。提交前请保存正文。风格、配色与视觉设定保持一致。",
            "bindFirst": "请在文章列表顶部选择设计风格。",
            "unavailable": "配图生成暂不可用。",
            "history": "历史版本",
            "status": {
                "analyzing": "分析中",
                "partial": "部分完成",
                "empty": "无可用场景",
                "pending": "等待中",
                "submitting": "提交中",
                "processing": "生成中",
                "succeeded": "已完成",
                "failed": "失败"
            },
            "processing": "正在生成配图，可关闭窗口稍后查看。",
            "failed": "配图未能完成，可重新生成。",
            "uncertain": "未能确认模型提交结果，已停止自动重试。再次生成将发起新的付费请求。",
            "empty": "选择全文或提供选段，生成延续文章视觉风格的无文字配图。",
            "loadFailed": "配图历史加载失败，请刷新。",
            "submitFailed": "配图提交失败，请重试。",
            "regenerate": "重新生成",
            "snapshot": "查看本版内容与生成设置",
            articleHint: "从已保存的全文提取场景。",
            maxImagesHint: "最多 1–5 张，默认 5 张；张数是上限，不强凑。",
            reference: "视觉参考：{kind} {id}",
            noReference: "共享视觉设定",
        },
        infographic: {
            articleHint: "从已保存的全文提炼信息。",
            analysisFailed: "文章分析未能完成，未提交生图。可重新分析。",
            "selectionHint": "根据选段提炼一张信息图。",
            "priceHint": "每张 {price} 积分，最多 {total} 积分；按成功张数计费。",
            "analyzing": "正在分析内容并拆分卡片…",
            "progress": "已完成 {completed} / {total} 张，失败 {failed} 张",
            "partial": "已完成 {completed} 张，失败 {failed} 张；已完成图片可下载。",
            "noCards": "没有找到适合生成信息图的内容，未生成图片。",
            "cardEvidence": "来源与选择理由",
            "enlarge": "放大查看",

            "source": "内容来源",
            "sources": {
                "article": "整篇文章",
                "selection": "选段"
            },
            "sourceText": "选中或粘贴真实内容（最多 20,000 字）",
            "orientation": "画面方向",
            "orientations": {
                "square": "方形",
                "landscape": "横向",
                "portrait": "纵向"
            },
            "language": "语言",
            "generate": "生成信息图",
            "credits": "积分",
            "hint": "整篇文章先分析并拆分卡片，最多生成所选张数；选段生成一张。沿用已锁定设计，请先保存正文，生成后核对事实与文字。",
            "bindFirst": "请在文章列表顶部选择设计风格。",
            "unavailable": "信息图生成暂不可用。",
            "history": "历史版本",
            "status": {
                "analyzing": "分析中",
                "partial": "部分完成",
                "empty": "无适合内容",
                "pending": "等待中",
                "submitting": "提交中",
                "processing": "生成中",
                "succeeded": "已完成",
                "failed": "失败"
            },
            "processing": "正在生成信息图，可关闭窗口稍后查看。",
            "failed": "信息图未能完成，可重新生成。",
            "uncertain": "未能确认模型提交结果，已停止自动重试。再次生成将发起新的付费请求。",
            "empty": "选择内容来源、方向和语言，生成第一张信息图。",
            "loadFailed": "信息图记录读取失败，请刷新。",
            "submitFailed": "提交未完成，请重试或刷新查看状态。",
            "regenerate": "重新生成",
            "snapshot": "查看本版内容与生成设置"
        },
        "cover": {
            "position": "标题位置",
            "size": "输出尺寸",
            "positions": {
                "center": "居中",
                "top": "居上",
                "bottom": "居下"
            },
            "presets": {
                "blog": "博客横图",
                "wechat": "公众号宽图",
                "portrait": "竖版封面",
                "video": "视频横图",
                "square": "方形"
            },
            "generate": "生成封面",
            "credits": "积分",
            "hint": "先选择输出尺寸并保存文章标题。AI 按当前比例生成，设计并绘制标题字体；核心内容居中。",
            "bindFirst": "请在文章列表顶部选择设计风格。",
            "unavailable": "封面生成暂不可用，请稍后重试。",
            "history": "历史版本",
            "status": {
                "pending": "等待中",
                "submitting": "提交中",
                "processing": "生成中",
                "succeeded": "已完成",
                "failed": "失败"
            },
            "processing": "正在生成封面，可关闭窗口稍后查看。",
            "failed": "封面未能完成，可重新生成。",
            "uncertain": "未能确认模型提交结果，已停止自动重试。再次生成会发起新的付费请求。",
            "download": "下载",
            "generatedPosition": "本版标题位置",
            "empty": "选择尺寸和标题位置，生成第一张封面。",
            "loadFailed": "封面记录读取失败，请刷新。",
            "submitFailed": "提交未完成，请重试或刷新查看状态。",
            "regenerate": "重新生成",
            "generatedSize": "生成尺寸",
            "resizeHint": "当前预览按新尺寸裁切；点击重新生成可让 AI 按新比例重新构图。",
            "previewLoading": "正在处理图片…",
            "previewFailed": "图片处理失败，请重试。"
        },
        "title": "AI 插图",
        "description": "封面、信息图和配图。",
        "saveArticleFirst": "请先保存文章。",
        "loading": "加载中…",
        "error": "操作失败，请重试。",
        "design": "设计风格",
        "color": "配色",
        "locked": "已锁定",
        "lockedHint": "本文后续的新插图将沿用此设计。使用其他风格需要创建新文章。",
        "palettePreview": "配色预览",
        "previewTitle": "文章标题",
        "previewHint": "此处仅预览颜色搭配，不代表 AI 生成效果。",
        "confirmLock": "我确认使用此风格和配色，绑定后无法更改或删除。",
        "bind": "确认并锁定设计",
        "retry": "重新准备",
        "kindLabel": "插图类型",
        "notAvailable": "配图将在后续阶段开放。",
        "generateLater": "生成图片 · 待开放",
        "status": {
            "pending": "设计准备中…",
            "processing": "设计准备中…",
            "ready": "设计已就绪",
            "failed": "设计准备失败，请重试"
        },
        "scenes": {
            "work-report": "工作汇报",
            "knowledge-sharing": "知识分享",
            "product-launch": "产品发布"
        },
        "kinds": {
            "cover": {
                "name": "封面",
                "description": "结合文章标题建立主题视觉，由 AI 一并绘制标题。"
            },
            "infographic": {
                "name": "信息图",
                "description": "整理文章内容，用清晰的图文结构解释信息。"
            },
            "decorative": {
                "name": "配图",
                "description": "以基本无文字的画面延续文章视觉 IP，与封面保持一致。"
            }
        }
    },
    common: {
        doing: "Doing 中...",
        comingSoon: "敬请期待...",
        notFound: "页面未找到",
        notFoundDesc: "无法找到对应的功能模块",
        version: "版本",
        generating: "生成中...",
        submit: "提交",
        cancel: "取消",
        confirm: "确认",
        delete: "删除",
        edit: "编辑",
        save: "保存",
        saving: "保存中...",
        preview: "预览",
        or: "或",
        unknownError: "未知错误",
        zh: "中文",
        en: "English",
        account: "账号管理",
        aiHelp: "AI 帮写",
        refresh: "刷新",
        close: "关闭",
        feedback: "反馈",
        feedbackButton: "反馈",
        feedbackTitle: "意见反馈",
        feedbackLoading: "加载中...",
        feedbackRewardValue: "100 积分",
        feedbackRewardAriaLabel: "提交反馈，赠送 100 积分",
        cookiePreferences: "Cookie 偏好设置",
        themeSwitcher: {
            tooltip: "切换主题",
            title: "选择界面主题",
            subtitle: "不同写作场景使用不同氛围，设置会自动保存。",
            blueWhite: "蓝白",
            blueWhiteDesc: "清爽明亮，适合日常整理",
            blackGold: "黑金",
            blackGoldDesc: "低光沉浸，适合夜间写作",
            paper: "纸张",
            paperDesc: "温暖纸感，适合长文编辑",
        },
        dateRange: {
            selectDate: "选择日期范围",
            from: "开始日期",
            to: "结束日期",
            quickSelect: "快捷选择",
            today: "今天",
            yesterday: "昨天",
            last7Days: "最近7天",
            last30Days: "最近30天",
            thisWeek: "本周",
            lastWeek: "上周",
            thisMonth: "本月",
            lastMonth: "上月",
            thisYear: "今年",
            lastYear: "去年",
            custom: "自定义",
            clear: "清除",
            cancel: "取消",
            confirm: "确认",
        },
        infiniteScroll: {
            loading: "加载中...",
            noMoreData: "已加载全部数据",
            loadFailed: "加载失败，点击重试",
            error: "加载出错：{error}",
        },
    },
    sidebar: {
        title: "创作者工具箱",
        subtitle: "Content Creator Tools",
        joyfulWords: "JoyfulWords",
        contentWriting: "内容书写",
        imageGeneration: "图片生成",
        knowledgeCards: "知识卡片",
        seoGeo: "SEO/GEO",
        videoEditing: "基础视频剪辑",
        billing: "计费管理",
    },
    contentWriting: {
        title: "内容书写",
        subtitle: "内容创作与管理工具",
        tabs: {
            articleManager: "文章管理",
        },
        materials: {
            types: {
                all: "全部类型",
                info: "资料",
                news: "新闻",
                image: "图片",
            },
            toast: {
                fetchListFailed: "获取素材列表失败",
                deleteFailed: "删除素材失败",
                deleteSuccess: "素材删除成功",
                updateFailed: "更新素材失败",
                updateSuccess: "素材更新成功",
                createSuccess: "素材创建成功",
                createSuccessDesc: "素材 \"{name}\" 已成功添加到列表",
                createFailed: "创建素材失败",
            },
            filterName: "素材名称：",
            filterNamePlaceholder: "搜索素材名称...",
            filterType: "素材类型：",
            uploadBtn: "上传素材",
            table: {
                name: "素材名称",
                type: "素材类型",
                link: "素材链接",
                content: "素材内容",
                time: "创建时间",
                actions: "操作",
                noData: "暂无素材数据",
                clickToView: "点击查看完整内容",
                clickToViewLinks: "点击查看所有链接",
                links: "个链接",
            },
            preview: {
                imageTitle: "图片预览",
                textContent: "完整内容",
                linksTitle: "相关链接",
            },
            dialog: {
                editTitle: "编辑素材",
                editDesc: "修改素材的详细信息",
                uploadTitle: "上传素材",
                uploadDesc: "添加新的素材到您的素材库",
                deleteTitle: "确认删除",
                deleteDesc: "此操作无法撤销。确定要删除这条素材吗？",
                nameLabel: "素材名称",
                typeLabel: "素材类型",
                linkLabel: "素材链接",
                contentLabel: "素材内容",
                namePlaceholder: "请输入素材名称",
                contentPlaceholder: "请输入素材内容描述...",
                saveBtn: "保存修改",
                typeHintInfo: "上传文字资料和说明文档",
                typeHintImage: "上传图片素材（仅支持一张图片）",
                imageLabel: "图片文件",
                uploadHint: "点击上传或拖拽图片到此处",
                uploadFormatHint: "支持 PNG、JPG、JPEG 格式，最大 5MB",
                selectImageBtn: "选择图片",
                removeImageBtn: "移除",
                fileName: "文件名:",
            },
            errors: {
                invalidImageType: "请选择图片文件",
                imageTooLarge: "图片大小不能超过 5MB",
            },
            pagination: {
                perPage: "每页",
                items: "条",
                totalInfo: "共 {total} 条，第 {page} 页",
                pageOf: "/",
            },
        },
        emptyDraftGuide: {
            title: "不知道从哪开始？",
            description: "告诉 AI 你的选题和风格，先生成一版可以继续编辑的初稿。",
            dismiss: "知道了",
        },
        writing: {
            toast: {
                warning: "警告",
                autoSaveFailed: "自动保存失败",
                editorNotReady: "编辑器未初始化，请稍后再试",
                imageInserted: "图片已插入到编辑器",
                selectTextFirst: "请先选中文本",
                selectTextFirstDesc: "请在编辑器中选中要改写的文本",
            },
            exportMarkdown: "导出为 Markdown",
            exportHtml: "导出为 HTML",
            editorPlaceholder: "在此处编写您的文章内容...",
            uploading: "上传中...",
            uploadSuccess: "图片上传成功",
            uploadFailed: "图片上传失败",
            uploadError: "上传失败: {error}",
        },
        aiHelp: {
            title: "AI 帮写",
            description: "选择素材，并在文字风格或参考文章中二选一，让 AI 帮您创作文章",
            overwriteHint: "生成文章将覆盖现有文章的内容。",
            overwriteConfirmTitle: "确认覆盖当前正文？",
            overwriteConfirm: "生成的内容将覆盖当前文章的全部正文，且无法撤销。确定继续吗？",
            overwriteConfirmAction: "继续生成",
            materialLabel: "选择素材",
            materialPlaceholder: "搜索素材...",
            materialScopeAll: "所有",
            materialScopeArticle: "本文章",
            materialScopeFavorites: "收藏",
            styleLabel: "选择文字风格",
            styleHint: "请选择一种文字风格，或上传一篇要模仿的文章",
            styleRequired: "请选择一种文字风格，或上传一篇要模仿的文章",
            promptLabel: "您的需求",
            promptPlaceholder: "描述您希望文章写什么内容...",
            promptRequired: "请输入您的需求，或选择素材",
            selectedMaterials: "已选择 {count} 个素材",
            generatingBtn: "生成中...",
            cancelBtn: "取消",
            confirmBtn: "确认生成",
            success: "文章已创建，正在生成内容...",
            loadMaterialsFailed: "加载素材失败",
            generateFailed: "生成失败，请重试",
            fileUploadLabel: "上传要模仿的文章",
            fileUploadHint: "支持 PDF、PNG、JPG/JPEG 格式，最大 5MB。上传后将自动清空已选文字风格。",
            invalidFileType: "仅支持 PDF、PNG、JPG/JPEG 格式",
            fileTooLarge: "文件大小不能超过 5MB",
            uploadFailed: "文件上传失败，请重试",
            uploadSuccess: "文件上传成功",
            uploading: "上传中...",
            removeFile: "移除文件",
            uploadedFile: "已上传文件",
            uploadInProgress: "文件上传中，请稍候...",
            materialsLoading: "正在加载素材...",
            noMoreMaterials: "已加载全部素材",
            styles: {
                CalmAnalysis: {
                    label: "冷静分析型",
                    altLabel: "Calm Analysis",
                    description: "克制、清晰、稳定、有判断，但不煽情。",
                },
                EmpatheticDrive: {
                    label: "共情激励型",
                    altLabel: "Empathetic Drive",
                    description: "有温度、有力量、有立场，能帮助被压抑和被质疑的人。",
                },
                NarrativeNonfiction: {
                    label: "纪实叙事型",
                    altLabel: "Narrative Nonfiction",
                    description: "有画面、有节奏、有现场感，用故事自然带出观点。",
                },
                SharpCommentary: {
                    label: "犀利评论型",
                    altLabel: "Sharp Commentary",
                    description: "尖锐、直接、有火力、观点先行。",
                },
                BusinessInsight: {
                    label: "商业观察型",
                    altLabel: "Business Insight",
                    description: "专业、现实、有框架，不空谈情怀。",
                },
                SocialSpark: {
                    label: "轻快传播型",
                    altLabel: "Social Spark",
                    description: "节奏快、钩子强、金句密、易读易转发。",
                },
            },
        },
        createModeDialog: {
            manual: {
                defaultTitle: "未命名文章",
                success: "已创建空白文章，正在进入编辑器",
                failed: "创建文章失败，请重试",
            },
        },
        editorHeader: {
            newArticle: "新文章",
            cleanTooltip: "清除所有内容并返回创建模式",
            publish: "发布",
            publishing: "发布中...",
            publishTooltip: "保存当前内容并发布文章",
            publishSuccess: "文章已发布",
            publishFailed: "发布失败",
            exportTooltip: "导出文章",
            saveMetadataSuccess: "保存成功",
            saveMetadataFailed: "保存失败",
            saving: "保存中...",
            saved: "已保存",
            saveAsNewReversion: "保存为新版本",
            contentTooLarge: "文章内容过大，无法保存",
            backToList: "返回文章列表",
            themeTooltip: "切换编辑器主题",
            presentation: {
                start: "进入演示模式",
                exit: "退出演示",
                laserPointer: "激光笔",
                laserPointerOn: "关闭激光笔",
                laserPointerOff: "打开激光笔",
                keyboardHint: "按 L 切换激光笔，按 Esc 退出",
            },
            themes: {
                blueWhite: "蓝白",
                blackGold: "黑金",
                paper: "纸张",
            },
        },
        version: {
            historyVersions: "版本历史",
            currentVersion: "当前版本",
            selectedVersion: "选中版本",
            versionList: "版本列表",
            applyVersion: "应用版本",
            closeCompare: "关闭版本对比",
            createdAt: "创建时间",
            description: "描述",
            articleTitle: "标题",
            empty: "暂无版本记录",
            versionName: "版本 {id}",
            autoSaveVersionDetail: "自动保存版本 - {time}",
            deleteVersion: "删除版本",
            loadFailed: "加载失败",
            parseFailed: "解析失败",
            parseFailedDescription: "版本数据解析失败",
            rollbackSuccess: "回滚成功",
            rollbackFailed: "回滚失败",
            deleteSuccess: "删除成功",
            deleteFailed: "删除失败",
        },
        manager: {
            loadFailed: "加载文章失败",
            articleNotFound: "文章未找到",
            loading: "加载中...",
            toast: {
                loadFailed: "加载失败",
                deleteFailed: "删除文章失败",
                deleteSuccess: "文章删除成功",
                updateFailed: "更新文章失败",
                updateSuccess: "文章更新成功",
                statusUpdateFailed: "状态更新失败",
                statusUpdateSuccess: "状态更新成功",
                aiWriteStartFailed: "AI 写作启动失败",
                aiWriteStartedDesc: "文章正在生成中，请稍候...",
            },
            filterTitle: "文章标题：",
            filterStatus: "文章状态：",
            searchTitlePlaceholder: "搜索文章标题...",
            totalCount: "共 {total} 篇文章",
            status: {
                all: "全部状态",
                init: "初始化",
                published: "已发布",
                draft: "草稿",
                archived: "已归档",
            },
            category: {
                all: "全部分类",
            },
            tags: {
                all: "全部标签",
            },
            table: {
                title: "文章标题",
                content: "文章内容",
                status: "状态",
                images: "图片素材",
                materials: "其他素材",
                posts: "竞品文章",
                created: "创建时间",
                modified: "修改时间",
                actions: "操作",
            },
            clickForDetail: "点击查看详情",
            clickForEdit: "点击编辑",
            openEditor: "编辑正文",
            editTitleAction: "编辑标题",
            editMetadataAction: "编辑文章信息",
            desktopEditOnly: "正文编辑需要在电脑端操作，请切换到桌面浏览器继续。",
            workspaceSubtitle: "集中管理草稿、正文和创作任务，点击文稿即可进入编辑器。",
            searchAndFilter: "搜索与筛选",
            totalArticles: "全部文章",
            recentlyUpdated: "最近更新",
            emptyTitle: "暂无文章",
            emptySearchTitle: "没有找到匹配标题的文章",
            emptyFilteredTitle: "没有找到符合当前筛选条件的文章",
            perPage: "每页",
            items: "条",
            materialsCount: "{count} 个素材",
            titleUpdated: "标题已更新",
            metadataUpdated: "文章信息已更新",
        },
        taskProgress: {
            statusCompleted: "已完成",
            statusFailed: "失败",
            statusPending: "进行中",
            title: "任务进度",
            aiEdit: "AI 编辑",
            imageGeneration: "图片生成",
            noTasks: "暂无进行中的任务",
            empty: "暂无任务",
            removeTask: "删除记录",
            types: {
                aiEdit: "AI 编辑",
                imageGeneration: "图片生成",
            },
        },
        taskCenter: {
            title: "任务中心",
            subtitleGlobal: "查看任务进度与结果。",
            subtitleArticle: "当前文章的异步任务会在这里实时同步。",
            searchPlaceholder: "搜索任务摘要或状态...",
            listTitle: "任务列表（{count}）",
            emptyTitle: "暂无任务",
            emptyDescription: "最近没有符合筛选条件的任务。",
            selectTaskTitle: "选择一个任务",
            selectTaskDescription: "点击左侧任务即可查看完整详情。",
            detailTitle: "任务详情",
            detailLoading: "任务详情加载中…",
            detailLoadFailed: "获取任务详情失败，请稍后重试。",
            openArticle: "打开文章",
            copyToMaterials: "加入素材库",
            copyToMaterialsSuccess: "已成功加入素材库。",
            copyToMaterialsFailed: "加入素材库失败，请稍后重试。",
            deleteSuccessConfirm: "删除后将无法找回，请谨慎。",
            clearFinishedConfirm: "将删除所有已完成和失败的任务记录（已生成的内容不受影响），删除后无法找回。确定继续吗？",
            deleteFailed: "删除任务失败，请稍后重试。",
            filters: {
                type: "任务类型",
                status: "任务状态",
                allTypes: "全部类型",
                allStatuses: "全部状态",
            },
            types: {
                illustration_cover: "封面",
                illustration_infographic: "AI 信息图",
                illustration_artwork: "配图",

                article: "文章",
                image: "图片",
                infographic: "信息图",
                presentation: "演示文稿",
                echarts: "图表",
                podcast: "播客脚本",
                podcast_audio: "播客音频",
            },
            statuses: {
                queued: "等待生成",
                pending: "待处理",
                processing: "处理中",
                failed: "失败",
                success: "成功",
                succeeded: "成功",
            },
            taskTitles: {
                illustration_cover: "封面",
                illustration_infographic: "AI 信息图",
                illustration_artwork: "配图",

                articleEdit: "文章编辑任务",
                articleWrite: "AI 写作任务",
                articleWriteCreate: "新文章生成任务",
                articleWriteUpdate: "文章覆盖写作任务",
                generateImage: "图片生成任务",
                articleCover: "文章封面任务",
                articleFont: "字体图片任务",
                styleImage: "图片风格化任务",
                splitImages: "图片拆分任务",
                infographic: "信息图任务",
                presentation: "PPT 生成任务",
                echarts: "AI 图表任务",
                podcast: "播客脚本任务",
                podcastAudio: "播客音频任务",
            },
            taskSummaries: {
                presentationSlideCount: "{count} 页幻灯片",
                podcastAudioSegments: "已生成 {completed}/{total} 个音频分段",
            },
            fields: {
                execId: "执行 ID",
                scriptId: "脚本 ID",
                audioTaskId: "音频任务 ID",
                podcastType: "播客类型",
                language: "语言",
                title: "标题",
                summary: "摘要",
                stage: "阶段",
                storycardId: "Storycard ID",
                storycardVersion: "Storycard 版本",
                templateId: "模板 ID",
                slideCount: "页数",
                attempt: "尝试次数",
                createdAt: "创建时间",
                model: "模型",
                provider: "供应商",
                outputFormat: "输出格式",
                sampleRate: "采样率",
                revision: "版本",
                totalSegments: "总分段数",
                completedSegments: "已完成分段",
                failedSegments: "失败分段",
                cardName: "卡片名称",
                cardType: "卡片类型",
                prompt: "提示词",
                genMode: "生成模式",
                completedAt: "完成时间",
                updatedAt: "更新时间",
                verifyPassed: "PPTX 校验通过",
                verifyErrors: "PPTX 校验异常",
                pptUrl: "PPTX 地址",
                sourceText: "原始文本",
                resultText: "改写结果",
                writingPrompt: "写作要求",
                generatedContent: "生成结果",
                settlement: "结算状态",
                billing: "计费状态",
                error: "错误信息",
                outputImages: "结果图片",
                referenceImages: "参考图片",
            },
            boolean: {
                yes: "是",
                no: "否",
            },
            notCompleted: "尚未完成",
            verifyPassed: "PPTX 文件结构校验通过",
            writerUpdateReloadConfirm: "AI 写作已更新远端文章，是否重新加载？",
            settlement: {
                settled: "已结算",
                unsettled: "未结算",
            },
            billing: {
                noCharge: "本次不会扣费",
            },
            imageErrors: {
                img_gen_prompt_rejected: "你的提示词无法处理，请修改后重新生成。本次不会扣费。",
                img_gen_moderation_unavailable: "提示词审核服务暂时不可用，请稍后重试。本次不会扣费。",
                generation_failed: "图片生成失败。本次不会扣费，请稍后重试。",
            },
            imageErrorCta: "修改提示词后重新生成",
        },
        materialPanel: {
            sourceTitle: "素材管理器",
            search: "搜索",
            library: "素材库",
            searchPlaceholder: "搜索素材...",
            allMaterials: "全部素材",
            searchTab: "搜索",
            libraryTab: "素材库",
            globalFavoriteButton: "全局收藏",
            emptyLibrary: "暂无素材",
            emptyFavorites: "暂无收藏素材",
            typeInfo: "资料",
            typeNews: "新闻",
            typeImage: "图片",
            searchInputPlaceholder: "输入搜索关键词...",
            searchButton: "搜索",
            searchSubmitting: "正在提交搜索",
            searching: "AI 搜索中...",
            searchTriggered: "已提交搜索任务",
            searchingHint: "正在为“{query}”抓取与整理素材，完成前无法再次发起搜索。",
            cancelSearch: "取消搜索",
            searchInitialHint: "输入关键词搜索新素材",
            searchInitialDescription: "支持资料、新闻、图片三类结果。搜索完成后可多选导入到素材库。",
            clueBoardEntryTitle: "展开线索白板",
            clueBoardEntryDescription: "把当前搜索词变成探索根节点，点击节点里的链接继续追踪相关线索。",
            clueBoardOpen: "展开白板",
            clueBoardExploreTitle: "打开线索白板深度探索",
            clueBoardExploreDescription: "围绕“{query}”展开可追踪线索，白板打开后会自动启动首次搜索。",
            clueBoardTitle: "素材线索白板",
            clueBoardDescription: "输入搜索词生成根节点，点击 Markdown 链接继续探索。",
            clueBoardInputPlaceholder: "输入搜索词创建根节点...",
            clueBoardStart: "开始",
            clueBoardFollowUp: "追问",
            clueBoardFollowUpPlaceholder: "继续搜索...",
            clueBoardFollowUpSubmit: "提交追问",
            clueBoardFollowUpCancel: "取消追问",
            clueBoardAddToMaterial: "加入素材库",
            clueBoardAddedToMaterial: "已加入素材库",
            clueBoardAddMaterialFailed: "加入素材库失败",
            clueBoardExpandFailed: "线索扩展失败，请稍后重试",
            clueBoardEmptyTitle: "输入关键词开始探索",
            clueBoardEmptyDescription: "白板会先生成一个搜索根节点，随后可以点击节点里的链接持续展开新的搜索节点。",
            searchNoResults: "未找到相关素材",
            collectButton: "加入素材库",
            collected: "已采纳",
            searchFailed: "搜索请求失败",
            searchStatusFailed: "搜索失败，请重试",
            searchTimeout: "搜索超时，请重试",
            deleteCard: "删除",
            collapseContent: "收起内容",
            expandContent: "查看全部内容",
            publishedAt: "发布时间",
            viewSource: "查看来源",
            imageResultAlt: "搜索结果图片",
            openImage: "打开",
            aiImageEntryTitle: "没有合适的图片？",
            aiImageEntryDescription: "可以使用 AI 生成一张，并保存到素材库。",
            aiImageEntryAction: "AI 生成图片",
            resultCardTitle: "AI 搜索结果",
            searchCurrentPage: "第 {page} 页",
            previousPage: "上一页",
            nextPage: "下一页",
            importSelected: "导入所选（{count}）",
            importToLibrary: "加入素材库",
            importSuccess: "素材导入成功",
            importSuccessCount: "已导入 {count} 条素材到素材库",
            importPartialSuccess: "素材部分导入成功",
            importPartialSuccessDescription: "成功 {success} 条，失败 {failed} 条：{urls}",
            importFailed: "导入素材失败",
            importWorkerFailed: "后台导入失败，请稍后重试",
            libraryDeleteAction: "删除素材",
            libraryDeleteSuccess: "素材删除成功",
            libraryDeleteFailed: "删除素材失败",
            failedCardTitle: "搜索失败",
            failedCardDescription: "本次搜索未成功完成。删除卡片后可重新发起搜索。",
            noDataCardTitle: "未找到相关数据",
            noDataCardDescription: "本次搜索没有可用结果。删除卡片后可尝试更换关键词。",
            uploadButton: "上传素材",
            uploadDialogTitle: "上传素材",
            uploadMaterialName: "素材标题",
            uploadMaterialNamePlaceholder: "请输入素材标题",
            uploadMaterialType: "素材类型",
            uploadMaterialContent: "素材内容",
            uploadMaterialContentPlaceholder: "请输入素材内容",
            uploadSelectImage: "选择图片",
            uploadImageHint: "支持 PNG、JPEG 格式，最大 5MB",
            uploadSelectData: "选择资料文件",
            uploadDataHint: "支持 PDF、图片、Docx、PPTx、Xlsx，最大 50MB",
            uploadSubmit: "上传",
            uploadParseSubmit: "上传并解析",
            uploadParseStarted: "已开始解析",
            uploadParsing: "正在解析文档，完成后会在这里显示可编辑结果。",
            uploadParsedContent: "解析内容",
            uploadParseFailed: "解析失败",
            uploadParseFailedWithCode: "解析失败（错误码 {code}）",
            addToMaterials: "加入素材库",
            uploadCancel: "取消",
            uploadSuccess: "上传成功",
            uploadFailed: "上传失败",
            uploadNameRequired: "请输入素材标题",
            uploadContentRequired: "请输入素材内容",
            uploadImageRequired: "请选择图片文件",
            uploadDataRequired: "请选择资料文件",
            uploadDataInvalid: "仅支持 PDF、图片、Docx、PPTx、Xlsx 文件",
            uploadDataTooLarge: "资料文件不能超过 50MB",
            favoriteAction: "收藏素材",
            favoritedAction: "已收藏",
            favoriteSuccess: "收藏成功",
            favoriteFailed: "收藏失败",
            unfavoriteSuccess: "已取消收藏",
            unfavoriteFailed: "取消收藏失败",
            pinAction: "置顶收藏",
            unpinAction: "取消置顶",
            pinSuccess: "置顶成功",
            pinFailed: "置顶失败",
            unpinSuccess: "已取消置顶",
            unpinFailed: "取消置顶失败",
            favoriteDeleteAction: "删除收藏",
            favoriteDeleteSuccess: "收藏删除成功",
            favoriteDeleteFailed: "删除收藏失败",
        },
        articleDialogs: {
            editMetadata: {
                title: "编辑文章信息",
                titleLabel: "标题",
                titlePlaceholder: "请输入文章标题",
                categoryLabel: "分类",
                categoryPlaceholder: "输入分类或选择已有分类",
                tagsLabel: "标签",
                tagsPlaceholder: "输入标签后按 Enter 或逗号添加",
                tagsHint: "标签会自动去空格并去重",
                removeTag: "移除标签 {tag}",
            },
            editTitle: {
                title: "编辑文章标题",
                label: "标题",
                placeholder: "请输入文章标题",
                hint: "按 Enter 保存，按 Esc 取消",
            },
            deleteConfirm: {
                title: "确认删除",
                confirmBtn: "删除文章",
                cancelBtn: "取消",
            },
            publishManagement: {
                title: "发布管理",
                statusTitle: "Doing 中...",
                statusDesc: "发布管理功能正在开发中，敬请期待",
                confirmBtn: "我知道了",
            },
            translation: {
                title: "多语言翻译",
                statusTitle: "Doing 中...",
                statusDesc: "多语言翻译功能正在开发中，敬请期待",
                confirmBtn: "我知道了",
            },
            materialsLinks: {
                title: "其他素材",
                type: "类型",
            },
            posts: {
                title: "竞品文章",
                platform: "平台",
                viewOriginal: "查看原文",
            },
        },
    },
    imageGeneration: {
        title: "图片生成",
        subtitle: "AI 驱动的图片生成工具",
        placeholder: "描述你想要生成的图片...",
        uploadRef: "上传参考图",
        examples: "案例参考",
        toolbar: {
            select: "选择",
            rectangle: "矩形",
            delete: "删除",
            template: "模板",
            templateHint: "快速加载常用图片结构",
            tools: "画布区域",
            reset: "重置画布",
        },
        modeTabs: {
            creation: "创作",
            style: "风格",
            inversion: "反向",
            history: "生成记录",
        },

        // 新增：模型选择器
        model: {
            title: "AI 模型",
            selectPlaceholder: "选择模型",
            noModelsAvailable: "暂无可用模型",
            fetchFailed: "加载模型失败",
            loginRequiredTitle: "登录后可使用高级模型",
            loginRequiredDescription: "游客试用仅支持 nano-banana-2-fast。",
        },

        cover: {
            title: "AI 封面",
            description: "一键自动生成文章封面，快速完成背景、标题字体与导出参数配置。",
            titleOptimizeConfirm: "当前标题偏长，可能不适合封面排版。点击“确定”由 AI 优化为更短的标题（会同步更新文章标题），点击“取消”继续使用当前标题。",
            articleRequired: "请先保存文章后再创作封面",
            export: "导出封面",
            canvasStatus: "实时预览",
            previewPlaceholder: "输入封面标题",
            titleLabel: "标题",
            titlePlaceholder: "输入封面标题",
            generateTitle: "生成标题",
            fontModes: {
                preset: "预设字体",
                custom: "自定义字体",
                task: "任务图片",
            },
            tabs: {
                title: "标题",
                background: "背景",
                crop: "裁剪",
            },
            panels: {
                background: "背景设置",
                title: "标题与字体",
                crop: "裁切尺寸",
            },
            fontFamilyLabel: "字体",
            fonts: {
                system: "现代黑体",
                headline: "醒目标题",
                serif: "杂志衬线",
                song: "宋体",
                kai: "书写楷体",
                mono: "等宽字体",
            },
            customFontPresets: {
                shock: "震惊体",
                shockPrompt: "震惊体，粗黑描边，高对比，强烈网感标题字，字形夸张但清晰可读",
                handwriting: "书写体",
                handwritingPrompt: "自然手写体，笔触流畅，有轻微墨迹变化，亲切但精致",
                cartoon: "卡通体",
                cartoonPrompt: "卡通泡泡字，圆润饱满，轻微立体高光，活泼可爱",
            },
            customFontDescriptionLabel: "字体描述",
            customFontDescriptionPlaceholder: "描述你想要的字体风格，例如：复古报纸标题、科技霓虹字、毛笔飞白...",
            generateFont: "生成字体图片",
            fontTaskImageTitle: "字体任务 #{id}",
            noFontTaskImages: "暂无可用的字体图片任务",
            refreshFontTaskImages: "刷新字体任务",
            fontTaskImageLoadFailed: "字体任务图片加载失败",
            fontResizeHandle: "缩放标题",
            fontWeightLabel: "标题粗细",
            textColorLabel: "文字颜色",
            positionLabel: "位置",
            resetPosition: "重置",
            positionActions: {
                centerX: "左右居中",
                centerY: "上下居中",
                left: "左对齐",
                right: "右对齐",
            },
            backgroundModes: {
                solid: "纯色",
                gradient: "渐变",
                unsplash: "Unsplash",
                material: "素材库",
            },
            solidColorLabel: "背景色",
            gradientFromLabel: "起始色",
            gradientToLabel: "结束色",
            gradientDirectionLabel: "渐变方向",
            gradientDirections: {
                diagonal: "斜向",
                vertical: "纵向",
                horizontal: "横向",
                reverse: "反斜向",
            },
            unsplashPlaceholder: "输入图库关键词，例如 AI、城市、阅读",
            search: "搜索",
            unsplashBy: "作者：{name}",
            materialLoadFailed: "素材加载失败，请稍后重试",
            materialSearchPlaceholder: "搜索素材库图片",
            aiImage: "AI 生图",
            imageCreatorTitle: "图片创作",
            refreshMaterials: "刷新素材",
            platformLabel: "平台尺寸",
            widthLabel: "宽度",
            heightLabel: "高度",
            presets: {
                blog: "博客封面",
                wechat: "公众号封面",
                xhs: "小红书竖图",
                video: "视频横封面",
                square: "方形社媒",
                custom: "自定义",
            },
            altText: "文章封面",
            activeTask: "任务 #{id} 正在生成：{status}",
            resourcesLoading: "正在加载封面资源…",
            toast: {
                articleRequired: "请先保存文章",
                titleRequired: "请填写标题",
                titleGenerated: "标题已生成",
                titleGenerateFailed: "标题生成失败",
                modelRequired: "请选择生图模型",
                fontDescriptionRequired: "请描述自定义字体",
                fontTaskCreated: "字体生成任务已创建",
                fontTaskCreateFailed: "创建字体任务失败",
                fontGenerated: "字体图片已生成",
                fontGenerateFailed: "字体图片生成失败",
                fontLoadFailed: "字体图片加载失败",
                aiImageGenerated: "AI 图片已生成并加入素材库",
                backgroundLoadFailed: "背景图片加载失败",
                resourcesNotReady: "封面资源尚未加载完成，请稍候再保存",
                materialLoadFailed: "素材加载失败",
                unsplashQueryRequired: "请输入图库关键词",
                unsplashNoResultsSearchAgain: "找不到图片，请重新搜索",
                unsplashSearchFailed: "Unsplash 图片搜索失败",
                exported: "封面已插入文章并加入素材库",
                editorInserted: "封面已插入文章第一行",
                exportFailed: "封面导出失败，请稍后重试",
            },
        },

        canvas: {
            advancedMode: "高级模式",
            addLayerFirst: "请先添加至少一个图层",
            addPromptOrLayerFirst: "请先填写画面描述或添加图层",
            promptLabel: "描述你想生成的图片",
            promptPlaceholder: "例如：一张适合公众号封面的图片，清晨书桌上有一杯咖啡、笔记本和柔和窗光，画面干净、有高级感...",
            layoutAreas: "区域",
            status: "状态",
            ready: "可生成",
            waiting: "待描述",
            toolHints: {
                select: "点击选择图层，拖动移动位置",
                rectangle: "点击画布添加矩形",
                delete: "点击图层即可删除，可连续删除",
            },
            generateImage: "生成图片",
            generating: "生成中...",
            viewOriginal: "查看原图",
            downloadImage: "下载此图片",
            saveToMaterials: "保存到素材库",
            laterImplementation: "待实现",
            showGeneratedImage: "显示生成的图片",
            emptyState: {
                title: "画布空白",
                description: "直接填写上方描述生成，或从左侧添加区域精细控制画面",
            },
            layerLabel: "图层 {number}",
        },
        properties: {
            description: "描述",
            descriptionPlaceholder: "输入图层描述...",
            descriptionRequired: "描述为必填项，用于生成图片内容",
            // 节标题
            metadata: "画面尺寸",
            selectedLayer: "当前区域",
            globalStyle: "风格与色调",
            composition: "镜头与灯光",

            // 元数据
            sizePreset: "常用尺寸",
            width: "宽度",
            height: "高度",
            seed: "随机种子",
            seedHint: "设置为 -1 表示随机种子",
            model: "AI 模型",
            sizePresets: {
                landscape: "横版",
                portrait: "竖版",
                square: "方形",
            },

            // 图层属性
            descriptionLabel: "描述",
            descriptionPlaceholder2: "请输入图层描述...",
            descriptionRequired2: "必填项",
            referenceImage: "参考图片",
            referenceImageOptional: "(可选)",
            selectReferenceImage: "选择参考图片",
            selectImageFromMaterials: "从素材库选择",
            referenceUpload: "上传本地参考图",
            referenceUploadReplace: "替换参考图",
            referenceUploadFailed: "参考图上传失败",
            noImageMaterials: "暂无图片素材，请先在素材模块上传",
            imageSelected: "已选择: ",
            materialsLoading: "正在加载素材...",
            noMoreMaterials: "已加载全部素材",
            zIndex: "层级",
            infoBox: "画布区域用于精细控制局部内容；只想快速出图时，填写上方描述即可。",

            // 全局样式
            medium: "艺术媒介",
            selectMedium: "选择艺术媒介",
            style: "艺术风格",
            selectStyle: "选择艺术风格",
            colorAccent: "色调",
            selectColorAccent: "选择色调",

            // 艺术媒介选项
            mediums: {
                photography: "摄影",
                digitalIllustration: "数字插画",
                oilPainting: "油画",
                watercolor: "水彩",
                render3d: "3D 渲染",
                sketch: "素描",
                glass: "玻璃",
            },

            // 艺术风格选项
            styles: {
                renaissance: "文艺复兴",
                impressionism: "印象派",
                surrealism: "超现实主义",
                minimalism: "极简主义",
                baroque: "巴洛克",
                ukiyoe: "浮世绘",
                vaporwave: "蒸汽波",
                cyberpunk: "赛博朋克",
                ghibli: "吉卜力",
            },

            // 色调选项
            colorAccents: {
                monochrome: "单色",
                coolTones: "冷色",
                warmTones: "暖色",
                morandi: "莫兰迪",
                pastel: "粉彩",
                cinematic: "电影青橙",
                neon: "霓虹",
                earthTones: "大地色",
                highContrast: "高对比度",
            },

            // 构图设置
            camera: "相机",
            angle: "拍摄角度",
            selectAngle: "选择拍摄角度",
            angles: {
                eyeLevel: "水平视角",
                lowAngle: "低角度",
                highAngle: "高角度",
                topDown: "俯视",
                dutchAngle: "斜角",
            },
            focalLength: "焦距",
            selectFocalLength: "选择焦距",
            focalLengths: {
                "14mm": "14mm (超广角)",
                "35mm": "35mm (广角)",
                "50mm": "50mm (标准)",
                "85mm": "85mm (人像)",
                "200mm": "200mm (长焦)",
            },
            depthOfField: "景深",
            selectDepthOfField: "选择景深",
            depths: {
                shallow: "浅景深",
                deep: "深景深",
                macro: "微距",
            },

            // 灯光设置
            lighting: "灯光",
            type: "灯光类型",
            selectType: "选择灯光类型",
            types: {
                natural: "自然光",
                studio: "摄影棚",
                volumetric: "体积光",
                cinematic: "电影光",
                neon: "霓虹灯",
                rim: "轮廓光",
            },
            source: "光源位置",
            selectSource: "选择光源位置",
            sources: {
                front: "正面",
                side: "侧面",
                topDown: "顶部",
                bottomUp: "底部",
                backlight: "背光",
            },
            intensity: "光照强度",
        },
        validation: {
            missingDescription: "图层描述不能为空",
            missingDescriptionDesc: "有 {{count}} 个图层缺少描述，请先填写描述后再进入高级模式",
            missingPromptOrLayer: "请先填写画面描述或添加图层",
        },
        jsonPreviewDialog: {
            title: "高级模式",
            description: "查看底层配置、微调随机种子，并获取 AI 优化后的提示词",
            jsonLabel: "Creator JSON",
            promptLabel: "专业提示词",
            seedLabel: "随机种子",
            seedHint: "默认值为 -1，表示每次直接生成都使用随机种子",
            convertButton: "转换提示词",
            convertingButton: "转换中...",
            copyButton: "复制",
            copiedButton: "已复制",
            generateImageButton: "生成图片",
            toast: {
                convertFailed: "提示词转换失败",
                convertSuccess: "提示词转换成功",
                copySuccess: "已复制到剪贴板",
            },
        },
        templates: {
            sideBySide: {
                title: "并列矩形",
                description: "适合左右分区，对比主体和补充信息",
                layers: {
                    primary: "主体视觉 / 产品主体",
                    secondary: "补充信息 / 品牌文案",
                },
            },
            nestedRectangles: {
                title: "嵌套矩形",
                description: "适合主画面里嵌一个重点局部或特写",
                layers: {
                    outer: "主画面 / 场景主体",
                    inner: "重点特写 / 细节放大",
                },
            },
            stackedRectangles: {
                title: "上下矩形",
                description: "适合上方主视觉、下方信息块的排版",
                layers: {
                    top: "上方主视觉 / 标题图",
                    bottom: "下方信息块 / 说明文案",
                },
            },
        },
        generating: {
            title: "图片生成中",
            description: "AI 正在生成图片，请稍候...",
            hint: "您可以切换到其他标签页或菜单，生成过程将继续进行",
            initiating: "正在提交生成任务...",
            resuming: "恢复生成任务...",
            started: "生成任务已创建",
            processing: "正在生成图片，预计需要 {{eta}} 秒...",
        },
        toast: {
            taskCreated: "生成任务已创建",
            taskCreateFailed: "创建生成任务失败",
            generationSuccess: "图片生成成功",
            generationFailed: "图片生成失败",
            timeout: "图片生成超时，请稍后重试",
            saveToMaterialsComingSoon: "永久保存到素材功能将在后期实现",
            comingSoon: "功能开发中",
            copyToMaterialsSuccess: "已成功复制 {count} 张图片到素材库",
            copyToMaterialsFailed: "复制到素材失败",
            error: {
                logNotFound: "图片生成记录不存在",
                notCompleted: "图片生成尚未完成，请等待生成完成后再试",
                noImages: "该记录没有可复制的图片",
                serverError: "服务器错误，请稍后重试",
                unauthorized: "请先登录",
                invalidFileType: "文件类型无效",
            },
            errorCodes: {
                imgGenPromptRejected: "图片描述未通过安全检查，请调整后重试",
                imgGenModerationUnavailable: "安全检查暂时不可用，请稍后重试",
                generationFailed: "图片生成失败，请稍后重试",
            },
        },
        reset: {
            dialogTitle: "重置画布",
            dialogDescription: "这将会把整个画布重置为初始状态。所有图层和设置将被清除。生成的图片会保留在图片生成记录里。",
            cancel: "取消",
            confirm: "重置",
            success: "画布已成功重置",
        },
        styleMode: {
            // 左侧栏 - 基础图片
            baseImage: {
                title: "基础图片",
                description: "请上传黑白线稿图，AI 将会把风格迁移，并保留图片主体",
                dropHere: "拖拽图片到此处",
                orClick: "或点击选择文件",
            },

            // 上传状态
            uploading: "上传中...",
            uploadingHint: "正在上传图片到服务器",

            // 验证错误
            validation: {
                missingInput: "信息不完整",
                missingInputDesc: "请先上传图片并选择风格",
                missingImage: "请先上传图片",
                missingStyle: "请选择风格",
                missingModel: "未选择模型",
                missingModelDesc: "请选择一个图片生成模型",
                uploadFailed: "图片上传失败",
            },

            // 右侧栏 - 风格列表
            styleList: {
                title: "Style List",
                custom: "自定义",
                customPlaceholder: "请输入风格描述（如：水彩画风格、油画风格...）",
                customConfirm: "确认",
                customCancel: "取消",
                loading: "加载风格中...",
                loadError: "加载风格失败",
                retry: "重试",
            },

            // 预览区域
            preview: {
                original: "原图预览",
                live: "实时预览",
                reupload: "重新上传",
                waitingForUpload: "等待上传",
                uploadHint: "上传图片并选择风格开始创作",
            },

            // 操作按钮
            generate: "生成图片",
            rendering: "AI 渲染中...",
            aiRendering: "AI 正在渲染",
            download: "下载图片",
            selected: "已选择",

            // 预设风格（保留以兼容，但不再使用）
            presets: {
                cyberNeon: {
                    name: "赛博霓虹",
                    description: "高饱和度霓虹色彩，未来科技感",
                },
                frostedGlass: {
                    name: "磨砂玻璃",
                    description: "半透明质感，柔和光晕",
                },
                minimalLine: {
                    name: "极简白描",
                    description: "黑白线条，简洁雅致",
                },
                warmOil: {
                    name: "温暖油画",
                    description: "厚重笔触，温暖色调",
                },
                anime: {
                    name: "二次元",
                    description: "日式动画风格，明亮色彩",
                },
                watercolor: {
                    name: "水彩晕染",
                    description: "流动色彩，自然融合",
                },
            },

            // 高级选项
            advancedOptions: "高级选项",
            styleStrength: "风格强度",
            subtle: "轻微",
            strong: "强烈",

            // 底部提示
            hint: "选择风格后点击「生成图片」按钮，AI 将自动为您的图片应用选定的风格效果。",
            thisMayTake: "这可能需要几秒钟",
        },
        inversionMode: {
            // 上传区域
            upload: {
                title: "图片上传",
                description: "上传需要拆分的图片",
                dropHere: "拖拽图片到此处",
                orClick: "或点击选择文件",
            },

            // 操作按钮
            splitImage: "拆分图片",
            splitting: "AI 正在拆分图片...",
            reupload: "重新上传",

            // 状态消息
            splitCompleted: "拆分完成！",
            successfullySplit: "成功拆分为 {count} 个图层",



            // 结果区域
            splitResults: "拆分结果",
            layersCount: "{count} 个图层",
            selectAll: "全选",
            deselectAll: "取消全选",
            downloadSelected: "下载选中",
            waiting: "等待拆分",
            waitingHint: "上传图片并点击拆分按钮后，这里将显示拆分后的图层",

            // 底部提示
            hint: "上传图片后点击「拆分图片」按钮，AI 将自动将图片分解为多个图层。",

            // 图层数量选择
            numLayers: {
                label: "拆分层级",
                description: "选择要拆分的图层数量（1-8层）",
            },

            // 场景描述
            prompt: {
                label: "场景描述",
                placeholder: "描述图片中的场景内容，例如：一个有山的风景画",
                description: "提供场景描述可以帮助 AI 更准确地拆分层级",
            },

            // 验证提示
            validation: {
                missingImage: "请先上传图片",
                invalidNumLayers: "图层数量必须在 1-8 之间",
            },

            // 图层默认命名
            layers: {
                default: "图层 {index}",
                defaultDescription: "拆分后的第 {index} 层",
                mainSubject: {
                    name: "主体层",
                    description: "图像的主要主体",
                },
                background: {
                    name: "背景层",
                    description: "图像的背景部分",
                },
                details: {
                    name: "细节层",
                    description: "图像的细节纹理",
                },
                lighting: {
                    name: "光影层",
                    description: "光照和阴影效果",
                },
            },
        },

        // 新增：提示词预览
        promptPreview: {
            title: "查看提示词",
            description: "查看本次生成使用的提示词",
            copy: "复制提示词",
            copied: "已复制！",
            copyFailed: "复制失败",
        },

        // 新增：生成记录
        logs: {
            filterStatus: "状态：",
            filterMode: "模式：",
            filterModel: "模型：",

            status: {
                all: "全部状态",
                pending: "等待中",
                processing: "生成中",
                success: "成功",
                failed: "失败",
            },

            mode: {
                all: "全部模式",
                creator: "创作",
                cover: "封面",
                font: "字体",
                style: "风格",
                split_images: "反向",
            },

            model: {
                all: "全部模型",
            },

            table: {
                time: "时间",
                mode: "模式",
                model: "模型",
                status: "状态",
                images: "图片",
                actions: "操作",
                noData: "暂无生成记录",
                clickToPreview: "点击预览",
            },

            actions: {
                viewPrompt: "查看提示词",
                download: "下载",
                copyToMaterials: "复制到素材",
                copying: "复制中...",
            },

            preview: {
                title: "图片预览",
                imageInfo: "图片 {current} / {total}",
            },

            toast: {
                fetchFailed: "获取生成记录失败",
            },
        },
    },
    knowledgeCards: {
        title: "知识卡片",
        subtitle: "将任意内容转换为结构化的知识卡片",
        configTitle: "配置知识卡片",
        contentLabel: "内容",
        contentPlaceholder: "请输入链接或文本内容（支持网页链接、文章、笔记等任意文本）",
        contentMinError: "内容至少需要10个字符",
        contentMaxError: "内容不能超过10000字符",
        contentDesc: "输入您想要转换成知识卡片的内容，可以是URL链接或任意文本",
        styleLabel: "卡片样式",
        layoutLabel: "卡片布局",
        langLabel: "语言",
        countLabel: "卡片数量",
        countDesc: "生成1-20张卡片（默认5张）",
        reqLabel: "卡片要求",
        reqPlaceholder: "可选：特殊要求或重点",
        reqDesc: "对生成的卡片有特殊要求可以在此说明",
        previewLabel: "样式预览",
        generateBtn: "生成知识卡片",
        resultTitle: "生成的知识卡片",
        printBtn: "打印卡片",
        downloadBtn: "下载HTML",
        toast: {
            generateSuccess: "成功生成 {count} 张知识卡片！",
            generateError: "生成卡片时发生错误，请重试",
        },
    },
    seoGeo: {
        title: "SEO/GEO",
        subtitle: "SEO 和地理配置优化工具",
        analysisTitle: "关键词分析",
        analysisDesc: "输入关键词获取详细的SEO分析报告",
        recommendationTitle: "推荐关键词",
        recommendationDesc: "相关的长尾关键词推荐",
        insightTitle: "市场洞察",
        insightDesc: "关键词市场趋势和竞争分析",
        searchInputPlaceholder: "请输入要分析的关键词...",
        analyzeBtn: "分析",
        analyzingBtn: "分析中...",
        startAnalysisTitle: "开始关键词分析",
        startAnalysisDesc: "输入您想要分析的关键词，获取详细的SEO分析报告，包括搜索量、竞争度、优化建议等。",
        keywordLabel: "关键词: ",
        analyzedAtLabel: "分析时间: ",
        insights: {
            searchTrendTitle: "关键词搜索量趋势",
            searchTrendDesc: "过去12个月的搜索量变化趋势",
            difficultyTitle: "关键词难度分布",
            difficultyDesc: "不同难度等级的关键词数量分布",
            competitorTitle: "竞争对手分析",
            competitorDesc: "主要竞争对手的SEO表现对比",
            stats: {
                totalKeywords: "总关键词数",
                avgSearchVolume: "平均搜索量",
                avgDifficulty: "平均难度",
                estTraffic: "预估流量",
                vsLastMonth: "较上月",
            },
            dataDescTitle: "数据说明:",
        },
        recommendations: {
            table: {
                rank: "排名",
                keyword: "关键词",
                msv: "MSV",
                difficulty: "难度",
                rating: "推荐指数",
                advantage: "核心优势",
                actions: "操作",
            },
            dialog: {
                title: "关键词详细分析: ",
                msvLabel: "月搜索量",
                difficultyLabel: "难度分数",
            },
            difficultyLevels: {
                easy: "简单",
                quiteEasy: "较易",
                medium: "中等",
                hard: "困难",
            },
            notesTitle: "说明:",
        },
    },
    auth: {
        login: "登录",
        signup: "注册",
        email: "邮箱",
        password: "密码",
        sendVerificationCode: "发送验证码",
        sendCodeHint: "我们将向您的邮箱发送6位验证码",
        back: "返回",
        verificationCode: "验证码",
        verificationCodeSent: "验证码已发送至",
        verificationCodePlaceholder: "请输入6位验证码",
        verificationCodeValid: "有效期15分钟",
        sending: "发送中...",
        resendInSeconds: "秒后重新发送",
        resend: "重新发送",
        newPassword: "新密码",
        newPasswordPlaceholder: "至少8位字符",
        confirmPassword: "确认密码",
        confirmPasswordPlaceholder: "再次输入密码",
        verifyAndSignup: "验证并注册",
        resetPassword: "重置密码",
        forgotPassword: "忘记密码？",
        noAccount: "还没有账号？",
        hasAccount: "已有账号？",
        continueWith: "或使用以下方式继续",
        continueAsGuest: "游客登录",
        continueWithGoogle: "使用 Google 登录",
        welcomeBack: "欢迎回来",
        loginSubtitle: "登录您的账号以继续",
        createAccount: "创建账号",
        signupSubtitle: "注册一个新账号开始使用",
        loginSuccess: "登录成功",
        redirecting: "跳转中...",
        passwordMismatch: "密码不匹配",
        passwordMismatchDescription: "请确保两次输入的密码相同",
        passwordStrength: {
            weak: "弱",
            fair: "一般",
            good: "良好",
            strong: "强",
            veryStrong: "非常强",
        },
        verifyEmailTitle: "验证您的邮箱",
        verifyEmailDescription: "我们已经向您的注册邮箱发送了验证邮件。请检查您的收件箱并点击验证链接以激活您的账号。",
        verifyEmailInstructions: "如果您在收件箱中没有看到邮件，请检查垃圾邮件文件夹或请求重新发送验证邮件。",
        verifyEmailNote: "注意：您需要验证邮箱后才能登录",
        backToLogin: "返回登录",
        forgotPasswordDescription: "输入您的邮箱地址，我们会向您发送重置密码的链接",
        profile: "个人资料",
        logout: "退出登录",
        changePasswordDescription: "修改密码后需要重新登录",
        oldPassword: "原密码",
        confirm: "确认",
        success: "成功",
        error: "错误",
        passwordChanged: "密码已修改，请重新登录",
        passwordTooShort: "密码长度至少需要8位",
        unknownError: "未知错误",
        noEmail: "暂无邮箱",
        signupComplete: "注册成功，请登录",
        passwordResetComplete: "密码重置成功，请登录",
        toast: {
            loginSuccess: "登录成功",
            loginFailed: "登录失败",
            googleLoginFailed: "Google 登录失败",
            sendFailed: "发送失败",
            verificationCodeSent: "验证码已发送",
            checkYourEmail: "请查收您的邮箱",
            signupSuccess: "注册成功",
            pleaseLogin: "请使用您的邮箱和密码登录",
            logoutSuccess: "已退出登录",
            resetFailed: "重置失败",
            passwordResetSuccess: "密码重置成功",
            loginWithNewPassword: "请使用新密码登录",
            loginWithCredentials: "请使用您的邮箱和密码登录",
            resetCodeSent: "如果该邮箱已注册，您将收到密码重置验证码",
            emailAlreadyRegistered: "该邮箱已注册",
            redirectedToLoginWithEmail: "已自动跳转到登录页，并为您填入邮箱",
            pleaseTryAgain: "请重试",
            googleOAuthUnavailable: "本地后端未配置 Google OAuth，请使用邮箱密码登录",
        },
        oauth: {
            missingParams: "缺少 OAuth 参数",
            stateVerificationFailed: "State 参数验证失败",
            stateMissing: "登录会话已失效，请重新使用 Google 登录",
            stateExpired: "Google 登录已超时，请重新发起登录",
            callbackInProgress: "Google 登录正在处理中，请稍候",
            loginFailed: "OAuth 登录失败",
            completingGoogleLogin: "正在完成 Google 登录...",
            processingLoginInfo: "请稍候，我们正在处理您的登录信息",
            loginSuccess: "登录成功！",
            redirecting: "正在跳转...",
            loginFailedTitle: "登录失败",
            redirectingToLogin: "正在跳转到登录页面...",
        },
        agentOAuth: {
            title: "授权命令行访问",
            subtitle: "确认后，命令行工具可以使用当前账号调用 JoyfulWords API。",
            authorizationRequest: "授权请求",
            authorizationDescription: "请确认客户端和设备信息。授权不会自动完成，必须手动批准。",
            codeLabel: "授权码",
            codeHint: "可直接输入 CLI 显示的 8 位代码，系统会自动格式化。",
            checkingLogin: "正在检查登录状态...",
            enterCodeTitle: "请输入授权码",
            enterCodeDescription: "打开 CLI 显示的授权链接，或手动输入代码后继续。",
            loadingAuthorization: "正在加载授权详情...",
            errorTitle: "授权请求不可用",
            clientName: "客户端",
            deviceName: "设备",
            expiresAt: "过期时间",
            permissions: "权限",
            apiFullDescription: "api:full 允许该 CLI 使用当前账号调用 JoyfulWords API。",
            authorizeClient: "授权 {clientName}",
            deny: "拒绝",
            errors: {
                invalidUserCode: "授权码无效，请检查 CLI 显示的代码。",
                authorizationExpired: "授权请求已过期，请回到 CLI 重新发起。",
                authorizationDenied: "授权已被拒绝。",
                alreadyConsumed: "授权请求已完成，请回到命令行。",
                invalidOrExpiredToken: "登录已过期，请重新登录。",
            },
            status: {
                approvedTitle: "授权完成",
                approvedDescription: "可以回到命令行继续。",
                deniedTitle: "授权已拒绝",
                deniedDescription: "命令行不会获得访问权限。",
                expiredTitle: "授权已过期",
                expiredDescription: "请回到 CLI 重新发起授权。",
                consumedTitle: "授权已完成",
                consumedDescription: "可以回到命令行继续。",
            },
        },
        mcpOAuth: {
            title: "授权 MCP 访问",
            subtitle: "确认后，MCP Server 可以使用当前账号访问已授权的 JoyfulWords API。",
            authorizationRequest: "MCP 授权请求",
            authorizationDescription: "请确认客户端、回调地址和权限范围。授权不会自动完成，必须手动批准。",
            checkingLogin: "正在检查登录状态...",
            loadingAuthorization: "正在加载授权详情...",
            invalidRequestTitle: "授权请求参数不完整",
            invalidRequestDescription: "缺少必要参数：{fields}",
            errorTitle: "授权请求不可用",
            clientName: "客户端",
            resourceName: "资源",
            redirectUri: "回调地址",
            permissions: "权限",
            callbackTitle: "批准后将跳转回 MCP Server",
            callbackDescription: "后端会生成一次性 authorization code，并把浏览器跳转到 MCP Server 提供的回调地址。",
            authorizeClient: "授权 {clientName}",
            deny: "拒绝",
            scopes: {
                articleRead: "article.read：允许读取文章列表和文章详情。",
                articleCreate: "article.create：允许创建文章。",
                articleUpdate: "article.update：允许更新文章内容。",
                unknown: "{scope}：后端返回的授权范围。",
            },
        },
        agreeToTerms: "我已阅读并同意",
        and: "和",
        termsRequired: "您必须同意所有条款才能继续注册",
    },
    fileConverter: {
        sampleMarkdown: `# 季度经营报告

## 核心结论

> 本季度增长主要来自自动化交付和重点客户续约。

1. 收入保持增长
2. 客户留存率改善
3. 下一阶段聚焦流程自动化

## 数据表

| 指标 | 本期 | 环比 |
| --- | --- | --- |
| 收入 | 1280 万 | +18% |
| 付费客户 | 420 | +12% |`,
        page: {
            eyebrow: "文档转换器",
            title: "文件转换",
            studioDescription: "文章 Markdown 转 Word，可套用 Word 模板样式。",
            studioTitle: "文章 Markdown 转 Word",
            toolboxMarkdownDescription: "粘贴 Markdown，选择 Word 模板后直接生成可编辑文档。",
            toolboxMarkdownTitle: "Markdown 转 Word",
            toolboxPptDescription: "上传 PPTX 文件，提取标题、要点和备注并转换为 Word。",
            toolboxPptTitle: "PPT 转 Word",
        },
        access: {
            authChecking: "检查登录状态",
            guestQuota: "游客模式 · 使用游客额度",
            signedIn: "已登录",
        },
        guest: {
            templateLoginTitle: "登录后上传个人模板",
            templateLoginDescription: "游客可使用系统默认和公共模板。",
            login: "登录",
            resultHint: "游客结果仅保存在当前游客身份下，请及时下载。",
        },
        modes: {
            markdownToWord: "Markdown 转 Word",
            pptToWord: "PPT 转 Word",
            pdfToWord: "PDF 转 Word",
        },
        actions: {
            generateWord: "生成 Word",
            startConversion: "开始转换",
            generating: "生成中",
            converting: "转换中",
            downloadWord: "下载 Word",
            download: "下载",
            refresh: "刷新",
            upload: "上传",
            remove: "移除",
        },
        source: {
            articleMarkdown: "文章 Markdown",
            markdown: "Markdown",
            articleMarkdownPreview: "文章 Markdown 预览",
            markdownInput: "Markdown 输入区",
            selectPpt: "选择 PPT 文件",
            selectPdf: "选择 PDF 文件",
            task: "任务 {id}",
            generatedWord: "已生成 Word 文件",
            templateSummary: "模板：{name}",
        },
        templates: {
            title: "Word 模板",
            selection: "模板选择",
            selectPlaceholder: "选择 Word 模板",
            systemDefault: "系统默认模板",
            publicPrefix: "公共 · ",
            minePrefix: "我的 · ",
            uploadTitle: "上传模板",
            namePlaceholder: "模板名称",
            selectDocx: "选择 .docx",
            defaultDescription: "可上传 Word 模板后查看原内容和样式。",
        },
        preview: {
            paragraphs: "{count} 段",
            tables: "{count} 表",
            images: "{count} 图",
            emptyBody: "模板没有可展示的正文内容。",
            hoverHint: "鼠标悬浮模板内容查看样式详情",
            imageLayout: "图片版式 {count}",
            roles: {
                heading1: "标题 1",
                heading2: "标题 2",
                heading3: "标题 3",
                quote: "引用样式",
                table: "标准业务表格样式",
                image: "图片版式",
                orderedList: "多级自动编号",
                normal: "正文",
            },
            style: {
                labels: {
                    style: "样式",
                    font: "字体",
                    size: "字号",
                    color: "色号",
                    bold: "加粗",
                    italic: "斜体",
                    alignment: "对齐",
                    lineSpacing: "行距",
                    spaceBefore: "段前",
                    spaceAfter: "段后",
                },
                defaultValue: "默认",
                inheritedValue: "继承",
                yes: "是",
                no: "否",
                alignments: {
                    left: "左对齐",
                    center: "居中",
                    right: "右对齐",
                    justify: "两端对齐",
                },
            },
        },
        errors: {
            templateReadFailed: "模板读取失败",
            unsupportedFormat: "文件格式不支持",
            selectPptx: "请选择 .pptx 文件。",
            selectPdf: "请选择 .pdf 文件。",
            selectDocx: "请选择 .docx 文件。",
            fileTooLarge: "文件过大",
            fileTooLargeDescription: "请上传 50MB 以内的文件。",
            selectWordTemplate: "请选择 Word 模板文件",
            templateParsed: "模板已解析",
            templateUploadFailed: "模板上传失败",
            conversionCompleted: "转换完成",
            conversionFailed: "转换失败",
            authenticationRequired: "请先登录后继续。",
            guestQuotaExceeded: "今日游客体验总额度已用完，请登录后继续。",
            requestFailed: "请求失败，请稍后重试。",
        },
    },
    tiptapEditor: {
        toolbar: {
            undo: "撤销",
            redo: "重做",
            bold: "粗体",
            italic: "斜体",
            underline: "下划线",
            strikethrough: "删除线",
            code: "行内代码",
            heading: {
                paragraph: "正文",
                heading1: "标题 1",
                heading2: "标题 2",
                heading3: "标题 3",
            },
            bulletList: "无序列表",
            orderedList: "有序列表",
            taskList: "待办列表",
            quote: "引用",
            codeBlock: "代码块",
            horizontalRule: "水平线",
            table: "表格",
            link: "链接",
            linkUrlPrompt: "输入链接 URL:",
            image: "图片",
            uploading: "上传中...",
            ai: "AI 帮写",
            aiWriting: "AI 写作中...",
        },
        toast: {
            editorNotReady: "编辑器未初始化，请稍后再试",
            uploadingImage: "正在上传图片...",
            imageInserted: "图片已插入到编辑器",
            selectTextFirst: "请先选中文本",
            selectTextFirstDesc: "请在编辑器中选中要改写的文本",
            saveBeforeAIRewrite: "请先保存文章",
            saveBeforeAIRewriteDesc: "请先保存文章，才能进入智能改写",
        },
        linkMenu: {
            open: "打开链接",
            edit: "编辑链接",
            delete: "删除链接",
            confirm: "确认",
            cancel: "取消",
            urlPlaceholder: "输入 URL...",
        },
        imageMenu: {
            widthLabel: "宽度:",
            styleImage: "风格化图片",
            styleListTitle: "选择图片风格",
            styleListDescription: "选择后会立即创建图片风格化任务。",
            loadingStyles: "加载风格中...",
            loadStylesFailed: "加载风格失败",
            retry: "重试",
            noStyles: "暂无可用风格",
            customStyle: "自定义风格",
            customTitle: "自定义图片风格",
            customDescription: "输入风格描述后创建图片风格化任务。",
            customPlaceholder: "例如：水彩插画风格，柔和色彩，保留主体结构...",
            customCancel: "取消",
            customSubmit: "创建任务",
            customRequired: "请输入自定义风格描述",
            noImageSource: "未找到当前图片地址",
            taskCreated: "图片风格化任务已创建",
            taskCreateFailed: "创建图片风格化任务失败",
            inpaint: "局部重绘",
            inpaintTitle: "局部重绘",
            inpaintSubtitle: "用半透明标注圈出需要重绘的区域。",
            inpaintModel: "模型",
            inpaintLoadingModels: "加载模型中",
            inpaintTool: "工具",
            inpaintPan: "平移",
            inpaintBrush: "画笔",
            inpaintErase: "橡皮擦",
            inpaintBrushSize: "画笔大小",
            inpaintStrokeList: "笔触列表",
            inpaintStrokeListEmpty: "暂无笔触",
            inpaintStrokeLabel: "笔触标签",
            inpaintRemoveStroke: "删除笔触",
            inpaintStrokeDefaultLabel: "笔触",
            inpaintUndo: "撤销",
            inpaintReset: "重置",
            inpaintClear: "清空笔触",
            inpaintConfirm: "确认并生成",
            inpaintNoMarks: "请先在这张图片上标注区域",
            inpaintUploading: "正在上传标注图...",
            inpaintGenerating: "正在生成图片...",
            inpaintReady: "生成完成，可继续重绘或回写编辑器。",
            inpaintGenerated: "局部重绘图片已生成",
            inpaintFailed: "局部重绘失败",
            inpaintImageLoadFailed: "图片加载失败",
            inpaintLoadingImage: "加载图片中",
            inpaintZoomHint: "重置视图后继续编辑",
            inpaintReplace: "替换原图",
            inpaintInsert: "插入新图",
            inpaintInserted: "已插入新图",
            inpaintReplaced: "已替换原图",
            inpaintInsertFailed: "插入新图失败",
            inpaintReplaceFailed: "替换原图失败",
            inpaintInsertedAlt: "局部重绘生成图",
        },
        aiPanel: {
            title: "AI 功能",
            studioTitle: "AI 工作室",
            groups: {
                writing: "文章增强",
                visual: "视觉生成",
                structure: "结构化输出",
            },
            groupDescriptions: {
                writing: "基于选中文本或全文进行改写、扩写、总结、信息图与图表生成。",
                visual: "为文章生成配图、拆分图层或统一风格。",
                structure: "把文章内容转成思维导图、播客音频、演示文稿或 Word 文档。",
            },
            aiEdit: "AI 编辑",
            aiWrite: "AI 写作",
            mindmap: "思维导图",
            aiCover: "AI 封面",
            createImage: "创作图片",
            aiGenerate: "AI 生成文章",
            reversalMode: "拆分图片图层",
            reverseMode: "拆分图片图层",
            imageStyle: "风格化图片",
            infographic: "AI 信息图",
            aiCharts: "AI 图表",
            generatePpt: "生成 PPT",
            wordConverter: "转 Word 格式",
            wordConverterDesc: "将当前文章整理为 Markdown 后生成 Word，并可套用 Word 模板样式。",
            wechatExport: "转微信公众号格式",
            podcastAudio: "转播客音频",
            generateVideo: "生成视频",
            generateVideoComingSoon: "功能开发中，敬请期待",
            featureUnavailable: "功能暂不可用",
            saveArticleFirst: "先保存文章后可用",
            clearFinished: "清除已完成",
            clearingFinished: "清除中...",
            taskProgress: "任务进度",

        },
    },
    podcastAudioDialog: {
        title: "文章转播客音频",
        description: "先生成可审核的播客脚本，再按分段生成音频并顺序播放。",
        saveArticleFirst: "请先保存文章，再生成播客音频。",
        settingsTitle: "播客设置",
        podcastTypeLabel: "播客类型",
        languageLabel: "语言",
        voiceSelectPlaceholder: "选择音色",
        voiceLoadFailed: "音色列表读取失败，请稍后重试。",
        previewVoice: "试听音色",
        generateScript: "生成播客脚本",
        regenerateScript: "重新生成脚本",
        saveScriptText: "保存脚本文字",
        generateAudio: "生成音频",
        regenerateAudio: "重新生成音频",
        downloadFullAudio: "下载完整音频",
        downloadMergedAudio: "下载 WAV 音频",
        mergeAudio: "正在合并音频",
        mergeProgress: "正在合并 {completed}/{total} 个音频分段",
        mergeFailed: "音频合并失败，请确认所有分段已生成完成后重试。",
        mergeUnavailable: "仍有音频分段未完成，暂不能下载完整音频。",
        footerMeta: "角色 {participants} · 脚本段落 {segments} · 预计时长 {duration} · 已生成音频 {audioSegments} 段",
        loadingLatest: "正在读取历史脚本...",
        scriptPreviewEyebrow: "Podcast Script",
        scriptEmptyTitle: "还没有播客脚本",
        scriptEmptyDescription: "点击生成播客脚本后，这里会展示标题、摘要、角色和逐句台词。",
        scriptReadonlyHint: "标题和摘要仅展示；可编辑每段台词，保存后需要重新生成音频。",
        segmentTextLabel: "台词",
        participantsLabel: "角色",
        segmentsLabel: "脚本段落",
        estimatedDurationLabel: "预计时长",
        playerEyebrow: "Audio Player",
        playerEmptyTitle: "等待音频生成",
        audioSegmentsTitle: "音频分段",
        segmentGenerating: "正在生成该分段音频...",
        segmentRegenerating: "正在重新生成该分段音频...",
        audioGeneratingEmpty: "音频生成中，完成的分段会出现在这里。",
        audioEmptyDescription: "脚本生成成功后，点击生成音频即可获得可播放的分段音频。",
        steps: {
            script: "生成播客脚本",
            audio: "生成播客音频",
        },
        podcastTypes: {
            news_broadcast: "新闻播报",
            two_person_interview: "双人访谈",
        },
        languages: {
            auto: "自动",
            "zh-CN": "中文",
            "en-US": "英文",
        },
        status: {
            pending: "排队中",
            processing: "处理中",
            success: "已完成",
            failed: "失败",
        },
        segmentFields: {
            speed: "语速",
            volume: "音量",
            pitch: "音高",
        },
        toast: {
            playFailed: "播放失败，请检查音频地址或浏览器权限。",
            scriptSaved: "脚本文字已保存",
            scriptSavedNoAudioDesc: "当前还没有可重生成的已完成音频分段。",
            scriptSavedSegmentAudioDesc: "已自动开始重生成这一段音频。",
            scriptSavedSegmentAudioFailedDesc: "这一段音频自动重生成失败，可使用右侧按钮按全文重新生成。",
            voicePreviewFailed: "音色试听失败，请稍后重试。",
            voicePreviewFailedDesc: "请更换音色或语言后重试。",
        },
    },
    wechatExport: {
        title: "微信公众号格式导出",
        description: "将当前文章转换为微信公众号编辑器可粘贴的富文本格式。",
        copy: "复制",
        copied: "已复制",
        empty: "当前文章暂无可导出的内容",
        meta: "{words} 字 · 约 {minutes} 分钟阅读",
        controls: {
            fontSize: "字号",
            caption: "图片说明",
            citeLinks: "引用外链为脚注",
            readingTime: "显示字数和阅读时间",
        },
        designLoading: "正在应用当前设计风格…",
        designError: "设计风格加载失败，请重试后复制。",
        caption: {
            alt: "使用图片 Alt",
            title: "使用图片 Title",
            none: "不显示",
        },
        toast: {
            copySuccess: "已复制为微信格式",
            copySuccessDesc: "可以直接粘贴到微信公众号编辑器。",
            copyFailed: "复制失败",
            copyFailedDesc: "浏览器剪贴板不可用，请重试或检查权限。",
        },
        hooks: {
            title: "开篇钩子",
            subtitle: "总结文章核心，并生成 7 类公众号开篇候选。",
            generate: "生成摘要",
            generating: "生成中...",
            summary: "120 字内总结",
            copy: "复制钩子",
            copySuccess: "钩子已复制",
            copyFailed: "复制钩子失败",
            toastSuccess: "摘要与钩子已生成",
            toastFailed: "生成失败",
            toastFailedDesc: "请稍后重试，或检查账户额度。",
            types: {
                hook_1: "反共识断言",
                hook_2: "真实数据冲击",
                hook_3: "个人故事开场",
                hook_4: "反问钩子",
                hook_5: "场景代入",
                hook_6: "痛点直击",
                hook_7: "反差对比",
            },
        },
    },
    echarts: {
        designLoading: "正在应用设计风格…",
        designRetry: "设计风格加载失败，点击重试",
        types: {
            bar: "柱状图",
            line: "折线图",
            pie: "饼图",
            unknown: "图表",
        },
        themes: {
            vintage: "复古",
            dark: "深色",
            macarons: "马卡龙",
            infographic: "信息图",
            shine: "明亮",
            roma: "罗马",
        },
        sort: {
            none: "默认",
            asc: "升序",
            desc: "降序",
        },
        orientation: {
            vertical: "纵向",
            horizontal: "横向",
        },
        positions: {
            top: "顶部",
            bottom: "底部",
            left: "左侧",
            right: "右侧",
        },
        alignments: {
            left: "左对齐",
            center: "居中",
            right: "右对齐",
        },
        actions: {
            reset: "重置",
        },
        display: {
            theme: "主题",
            sort: "排序",
            title: "标题",
            titlePosition: "标题位置",
            titleAlign: "标题对齐",
            legend: "图例",
            legendPosition: "图例位置",
            tooltip: "提示框",
            label: "数据标签",
            grid: "网格线",
            orientation: "方向",
            stack: "堆叠",
            radius: "圆角",
            barWidth: "柱宽",
            smooth: "平滑曲线",
            area: "面积填充",
            symbol: "显示节点",
            donut: "环形图",
            rose: "玫瑰图",
            percent: "显示占比",
        },
        chart: {
            untitled: "AI 图表",
            meta: "AI 图表 · {type}",
            editDisplay: "编辑展示设置",
            moreComingSoon: "扩展功能预留",
            displaySaved: "图表展示设置已更新",
            displaySaveFailed: "保存图表展示设置失败",
            generateFailed: "图表生成失败",
            generateFailedDesc: "请调整文本或提示词后重新生成。",
            emptySpec: "暂无图表数据",
        },
        dialog: {
            title: "AI 图表",
            selectionMode: "选中文本生成",
            selectionModeDesc: "基于当前选区生成，最多 {limit} 字；生成后在任务详情中调整样式并插入 PNG。",
            articleMode: "整篇文章分析",
            articleModeDesc: "生成任务会进入任务中心。",
            prompt: "生成要求",
            selectedText: "选中文本",
            selectedTextCount: "{count}/{limit} 字",
            selectedTextHint: "AI 图表会基于这段选中文本生成，最多 {limit} 字。",
            selectedTextTooLongHint: "当前选区 {count} 字，已超过 {limit} 字上限。请缩短选区，或切换到整篇文章分析。",
            selectionTooLongTitle: "选中文本过长",
            selectionTooLongDescription: "当前选区 {count} 字，最多只能提交 {limit} 字。",
            noSelection: "未选择文本",
            maxCharts: "最多图表数",
            generate: "生成图表",
            generateFromArticle: "分析整篇文章",
            selectedTextPrefix: "选中文本：",
            articleRequired: "请先保存文章后再生成整篇文章图表。",
            noChartsTitle: "没有找到适合图表化的内容",
            noChartsDescription: "后端未创建图表任务，可以换一篇文章或选中文本生成。",
            selectionSubmittedTitle: "AI 图表任务已创建",
            selectionSubmittedDescription: "任务正在后台生成，请在任务中心查看进度；完成后可调整样式并插入文章。",
            articleSubmittedTitle: "AI 图表任务已提交",
            articleSubmittedDescription: "已提交 {count} 个图表任务，请在任务中心查看进度。",
            analysisKeptTitle: "分析仍在进行",
            analysisKeptDescription: "状态已保留在右侧任务进度，重新打开 AI 图表即可继续查看。",
            analysisSubmittingTitle: "正在分析整篇文章",
            analysisSubmittingDescription: "AI 正在拆解文章结构并创建图表任务，完成后会自动进入任务进度。",
            analysisQueuedTitle: "文章分析任务已创建",
            analysisQueuedDescription: "正在后台提取图表建议，完成后会自动同步到任务进度。",
            analysisSubmittedInlineTitle: "图表任务已进入队列",
            analysisSubmittedInlineDescription: "已创建 {count} 个图表任务，可以在右侧任务进度中查看。",
            analysisFailedTitle: "文章分析失败",
            analysisFailedDescription: "文章分析未能完成，请稍后重试。",
            analysisTimeoutDescription: "文章分析等待超时，请稍后在任务进度中查看，或重新提交。",
            analysisTaskDescription: "正在分析文章，最多创建 {count} 个图表任务。",
            analysisSyncingDescription: "图表任务已创建，正在同步到任务进度。",
            maxChartsValue: "最多 {count} 个",
            articleAnalysisHint: "提交后可以关闭弹窗，分析状态会保留在任务进度中。",
        },
        taskDetail: {
            prompt: "生成提示词",
            regenerate: "重新生成图表",
            regenerateSubmitted: "已创建新的图表任务",
            regenerateFailed: "重新生成图表失败",
            insert: "插入 PNG 到文章",
            inserted: "已插入文章",
            insertedAtCursor: "未找到锚点，已插入当前光标位置",
            insertFailed: "插入图表失败",
            failed: "图表任务失败",
            waiting: "图表生成中，请稍后查看。",
            placement: "建议插入位置",
            displaySettings: "展示设置",
            displaySettingsHint: "调整后保存，再插入文章。",
            insertReady: "图表已准备好，可插入到当前文章。",
        },
    },
    presentationV2: {
        title: "生成 PPT",
        description: "确认内容结构，选择模板，然后生成可下载的 PowerPoint。",
        loading: "正在恢复 PPT 工作进度",
        back: "返回",
        languages: {
            zh: "中文",
            en: "English",
        },
        flow: {
            storycard: "Storycard",
            template: "模板与风格",
            generating: "生成",
            complete: "完成",
        },
        pageTypes: {
            "封面页": "封面页",
            "目录页": "目录页",
            "章节过渡页": "章节过渡页",
            "内容页": "内容页",
            "结尾页": "结尾页",
        },
        logicRelationLabels: {
            "并列": "并列",
            "递进": "递进",
            "对比": "对比",
            "包含": "包含",
            "四象限": "四象限",
            "时间轴": "时间轴",
            "循环": "循环",
            "总分": "总分",
            "金字塔": "金字塔",
            "因果": "因果",
            "图文": "图文",
            "图表": "图表",
            "趋势": "趋势",
            "占比": "占比",
            "表格": "表格",
        },
        storycard: {
            startTitle: "先生成 Storycard",
            startDescription: "系统会先把文章整理成可编辑的演示结构。确认内容后，再选择模板生成 PPT。",
            generatingTitle: "正在整理 Storycard",
            generatingDescription: "正在分析文章结构、章节关系和页面内容。这个过程可能需要几分钟。",
            failedTitle: "Storycard 生成失败",
            failedDescription: "可以重新提交生成请求；现有文章内容不会受到影响。",
            generate: "生成 Storycard",
            retry: "重新生成",
            slides: "页面结构",
            slideCount: "共 {count} 页",
            untitled: "未命名页面",
            overview: "演示概览",
            overviewDescription: "这些信息决定整份演示的受众、目标和叙事方向。",
            title: "演示标题",
            audience: "目标受众",
            goal: "演示目标",
            narrative: "叙事结构",
            slideNumber: "第 {index} 页",
            slideTitle: "页面标题",
            keyMessage: "核心信息",
            contentPoints: "内容要点",
            contentPointsHint: "每行一个要点",
            logicRelations: "推荐逻辑关系",
            logicRelationsHint: "按选择顺序作为模板匹配优先级，最多选择 3 个。",
            sourceRefs: "内容来源",
            noSourceRefs: "没有关联来源",
            save: "保存 Storycard",
            confirm: "确认并选择模板",
            unsaved: "有尚未保存的修改",
            saved: "Storycard 已保存",
            closeConfirm: "Storycard 还有未保存的修改，确定关闭吗？",
        },
        template: {
            title: "选择模板包",
            description: "模板包决定整份演示的视觉体系，具体页面会由系统根据 Storycard 自动匹配。",
            empty: "当前没有可用模板",
            noDescription: "暂无模板说明",
            viewLarge: "查看大图",
            previewCover: "查看{name}封面大图",
            privateTemplateTitle: "使用私有模板",
            privateTemplateDescription: "想用自己的模板？联系我们完成模板处理，之后即可用它生成任意内容的 PPT。",
            privateTemplateQrHint: "微信扫码，或点击放大",
            privateTemplateQrAlt: "联系我们处理私有模板的微信二维码",
            privateTemplateQrPreview: "查看私有模板联系二维码大图",
            generate: "使用所选模板与风格生成",
        },
        imageStyle: {
            title: "选择图片风格",
            description: "风格将应用于整份 PPT。系统会优先复用文章图片，并在图片不足时按此风格自动生成最多 3 张图片。",
            empty: "当前没有可用图片风格",
        },
        generation: {
            title: "正在生成 PowerPoint",
            failedTitle: "PPT 生成失败",
            progressLabel: "PPT 生成阶段",
            retry: "重试生成",
            editStorycard: "重新编辑 Storycard",
            jobId: "任务 ID",
            slideCount: "幻灯片",
            generatedImageCount: "AI 图片",
            statusLabel: "任务状态",
            status: {
                queued: "等待生成",
                processing: "生成中",
                succeeded: "生成完成",
                failed: "生成失败",
            },
            stage: {
                queued: "正在排队",
                preparing: "正在准备内容",
                cataloging_images: "正在检查文章图片",
                planning_images: "正在规划演示图片",
                generating_images: "正在生成演示图片",
                matching_templates: "正在分析页面结构",
                selecting_pages: "正在匹配模板页面",
                filling_deck_input: "正在组织演示内容",
                validating_input: "正在检查内容",
                resolving_assets: "正在准备图片和图标",
                generating_pptx: "正在生成 PowerPoint",
                verifying_pptx: "正在检查文件结构",
                uploading: "正在上传文件",
                succeeded: "PPT 已生成",
                failed: "生成失败",
            },
            stageProgress: "第 {current}/{total} 阶段",
        },
        taskCenter: {
            continueInArticle: "回到文章继续",
        },
        complete: {
            title: "PPT 已生成",
            description: "共生成 {count} 页幻灯片，可以下载并继续在 PowerPoint 中编辑。",
            download: "下载 PPTX",
        },
        validation: {
            firstSlideCover: "第一页必须是封面页。",
            lastSlideEnding: "最后一页必须是结尾页。",
            exactlyOneAgenda: "必须且只能有一张目录页。",
            agendaBeforeSections: "目录页必须位于章节过渡页之前。",
            sectionCount: "章节过渡页数量必须为 3–6 张。",
            agendaPointCount: "目录要点数量必须与章节数量一致。",
            contentSlideRequired: "至少需要一张内容页。",
            slideTitleRequired: "页面标题不能为空。",
            slideKeyMessageRequired: "核心信息不能为空。",
            invalidPageType: "页面类型不符合 Storycard 合同。",
            logicRelationsRequired: "内容页必须设置推荐逻辑关系。",
            logicRelationsCount: "推荐逻辑关系必须选择 1–3 个。",
            logicRelationInvalid: "包含不支持的逻辑关系。",
            logicRelationDuplicate: "推荐逻辑关系不能重复。",
            logicRelationsContentOnly: "只有内容页可以设置推荐逻辑关系。",
        },
        toast: {
            generateSubmitted: "Storycard 生成任务已提交",
            generateFailed: "Storycard 生成请求失败",
            saved: "Storycard 已保存",
            confirmed: "Storycard 已确认",
            versionConflict: "Storycard 已被更新，已载入最新版本",
            generationSubmitted: "PPT 生成任务已创建",
            retrySubmitted: "PPT 任务已重新排队",
        },
        errors: {
            saveArticleFirst: "请先保存文章，再生成 PPT。",
            loadStorycard: "无法载入 Storycard，请稍后重试。",
            loadGeneration: "无法载入 PPT 生成任务，请稍后重试。",
            loadTemplates: "无法载入 PPT 模板，请稍后重试。",
            loadImageStyles: "无法载入 PPT 图片风格，请稍后重试。",
            generateStorycard: "Storycard 生成请求失败，请稍后重试。",
            storycardTimeout: "Storycard 生成时间过长，请稍后重新打开查看。",
            validation: "Storycard 结构不符合生成要求，请先修正标记的问题。",
            versionConflict: "Storycard 已被其他操作更新，当前页面已载入最新版本，请确认后重试。",
            saveStorycard: "Storycard 保存失败，请稍后重试。",
            confirmStorycard: "Storycard 确认失败，请稍后重试。",
            createGeneration: "PPT 任务创建失败，请稍后重试。",
            retryGeneration: "PPT 任务重试失败，请稍后再试。",
            download: "PPTX 下载地址尚未就绪，请稍后重试。",
            generationMissing: "没有可恢复的 PPT 生成任务。",
            generation: {
                dependency_unavailable: "生成服务暂时不可用，请稍后重试。",
                invalid_storycard_snapshot: "Storycard 快照无效，请返回文章重新确认。",
                invalid_template_snapshot: "模板快照无效，请重新选择模板。",
                invalid_runtime_manifest: "PPT 运行时配置无效，请稍后重试。",
                article_not_found: "找不到来源文章，请返回文章列表确认。",
                image_catalog_failed: "文章图片整理失败，请稍后重试。",
                image_generation_failed: "演示图片生成流程失败，请稍后重试。",
                template_match_failed: "当前 Storycard 无法匹配模板，请调整内容结构后重试。",
                selected_pages_encode_failed: "模板页面数据处理失败，请稍后重试。",
                deck_fill_failed: "演示内容组织失败，请调整 Storycard 后重试。",
                deck_encode_failed: "演示内容数据处理失败，请稍后重试。",
                asset_resolution_failed: "图片或图标准备失败，请稍后重试。",
                template_download_failed: "模板下载失败，请稍后重试。",
                template_checksum_mismatch: "模板版本校验失败，请稍后重试。",
                runtime_failed: "PowerPoint 生成失败，请稍后重试。",
                runtime_output_invalid: "生成的 PowerPoint 文件无效，请稍后重试。",
                upload_failed: "PPTX 上传失败，请稍后重试。",
                submit_failed: "任务提交失败，请稍后重试。",
                unknown: "PPT 生成失败，请稍后重试。",
            },
        },
    },
    infographicDialog: {
        title: "生成信息图",
        description: "基于选中文本或整篇文章，生成适合传播的信息图。",
        sourceLabel: "内容来源",
        sourceSummaryLabel: "来源",
        selectionMode: "选中文本",
        selectionModeDesc: "使用当前编辑器选区生成，最多 {limit} 字。",
        articleMode: "整篇文章",
        articleModeDesc: "不选择文本时自动进入，分析全文并生成多张信息图。",
        selectedTextLabel: "当前选中内容",
        selectedTextPlaceholder: "可以直接编辑这段内容，用于生成信息图。",
        selectedTextCount: "{count}/{limit} 字",
        selectedTextHint: "信息图会基于这里的内容生成，最多 {limit} 字；可以直接修改后再提交。",
        selectedTextTooLongHint: "当前内容 {count} 字，已超过 {limit} 字上限。请缩短文本，或切换到整篇文章分析。",
        maxImagesLabel: "最多图片数",
        maxImagesValue: "最多 {count} 张",
        maxImagesHint: "整篇文章分析最多生成 5 张信息图，默认 5 张。",
        styleLabel: "视觉风格",
        orientationLabel: "画面方向",
        languageLabel: "语言",
        decorationLabel: "装饰密度",
        customLabel: "补充要求",
        customPlaceholder: "例如：强调步骤关系、突出关键词、增加数据对比",
        resultTitle: "生成结果",
        resultHint: "选择参数后开始生成，结果会显示在这里。",
        articleResultHint: "整篇文章分析会返回多张信息图结果。",
        resultEmptyTitle: "等待生成",
        resultEmptyDesc: "生成成功后，会在这里展示信息图图片。",
        articleResultEmptyTitle: "等待整篇分析",
        articleResultEmptyDesc: "提交后会展示多张候选信息图。",
        articleExcerptLabel: "文章片段：",
        selectionReasonLabel: "选择原因：",
        resultImageAlt: "信息图结果",
        previewImage: "点击预览大图",
        generate: "生成信息图",
        generateFromArticle: "分析整篇文章",
        regenerate: "重新生成",
        regenerateFromArticle: "重新分析整篇文章",
        generating: "生成中...",
        generatingHint: "正在轮询生成结果，请稍候。",
        generatingBatchHint: "正在轮询整篇文章信息图，已完成 {completed}/{total}。",
        restoringPreviousResult: "正在恢复当前文章上次生成的信息图。",
        restoringPreviousResultTitle: "正在恢复上次结果",
        restoringPreviousResultDesc: "正在读取最近一次成功生成的信息图，不会创建新任务。",
        restoredPreviousResult: "已恢复当前文章上次生成的信息图；不满意可重新生成。",
        addToMaterials: "加入素材库",
        addToMaterialsLoading: "加入素材中...",
        close: "关闭",
        status: {
            pending: "任务已创建，等待后端提交生图任务。",
            processing: "信息图生成中，正在持续查询结果。",
            success: "信息图生成完成。",
            failed: "信息图生成失败。",
            articleAnalyzing: "正在分析整篇文章，完成后将自动创建信息图任务。",
            batchProcessing: "整篇文章信息图生成中，已完成 {completed}/{total}。",
            batchSuccess: "整篇文章信息图生成完成，成功 {success}/{total}。"
        },
        styles: {
            professional: {
                title: "专业",
                description: "规整、可信、干净，适合知识表达。"
            },
            rustic: {
                title: "古朴",
                description: "沉静、雅致，适合人文与历史主题。"
            },
            academic: {
                title: "学术",
                description: "严谨清楚，带图解与论文摘要感。"
            },
            handdrawn: {
                title: "手绘",
                description: "像学习笔记，轻松但不随意。"
            },
            magazine: {
                title: "杂志",
                description: "有专题页气质，视觉冲击更强。"
            },
            minimal: {
                title: "极简",
                description: "克制留白，适合高级感表达。"
            },
            fresh: {
                title: "清新",
                description: "轻松亲和，适合大众传播。"
            }
        },
        orientations: {
            landscape: "横向",
            portrait: "竖向",
            square: "方形"
        },
        languages: {
            zh: "中文",
            en: "英文"
        },
        decorations: {
            simple: "简洁",
            moderate: "适中",
            rich: "丰富"
        },
        toast: {
            selectTextFirst: "请先输入文本",
            selectTextFirstDesc: "信息图需要基于这里的文本内容生成。",
            selectionTooLong: "文本过长",
            selectionTooLongDesc: "当前内容 {count} 字，最多只能提交 {limit} 字。",
            articleRequired: "缺少文章信息",
            articleRequiredDesc: "整篇文章分析需要当前文章 ID。",
            batchPolling: "文章分析任务已提交，正在分析并生成信息图。",
            noArticleImages: "没有找到适合生成信息图的内容",
            noArticleImagesDesc: "可以调整文章内容，或选中一小段文本生成单张信息图。",
            created: "信息图任务已创建",
            createFailed: "信息图创建失败",
            copySuccess: "已成功加入 {count} 个素材",
            copyPartialSuccess: "已加入 {count} 个素材，部分图片加入失败",
            copyFailed: "加入素材失败",
            pollingTimeout: "信息图生成超时，请稍后重试"
        }
    },
    asyncTaskToast: {
        submittingTitle: "{task}提交中",
        submittingDescription: "正在提交 {task} 任务，请勿退出当前页面。",
        pollingTitle: "{task}处理中",
        pollingDescription: "{task}任务已创建。可以随时退出，并在任务中心查看进度。"
    },
    aiRewrite: {
        title: "AI 智能改写",
        description: "选择改写方式，AI 将帮您优化内容",
        selectedText: "选中的文本",
        rewrittenText: "改写后的内容",
        rewrittenTextPlaceholder: "生成结果将在此显示...",
        markdownPreview: "预览",
        markdownSource: "Markdown",
        editMarkdown: "编辑 Markdown",
        finishMarkdownEditing: "完成编辑并预览",
        markdownEditor: "Markdown 内容编辑器",
        rewriteType: "改写功能",
        generating: "生成中...",
        generate: "生成",
        confirmApply: "确认应用",
        cancel: "取消",
        restoreOriginal: "恢复原文",
        resultReady: "AI 改写完成",
        waitingHint: "AI 正在改写中，请稍候…",
        waitingTaskHint: "任务处理中，状态会在这里自动更新。",
        waitingPlaceholder: "AI 改写完成后将自动显示…",
        submitting: "提交中…",
        waiting: "改写中…",
        failedTitle: "改写失败",
        failedHint: "任务未成功完成，请稍后重试。",
        types: {
            material: "素材扩充",
            style: "风格调整",
            struct: "结构优化"
        },
        material: {
            selectMaterials: "选择素材",
            selectPlaceholder: "请选择素材...",
            noMaterials: "暂无素材",
            loadingMaterials: "加载素材中...",
            loadingMore: "加载更多素材中...",
            noMoreData: "已加载全部素材",
            searchPlaceholder: "搜索素材...",
            scopeAll: "所有",
            scopeArticle: "本文章",
            scopeFavorites: "收藏",
            selectedCount: "已选择 {count} 个素材",
            contentFilters: {
                all: "全部",
                text: "文字",
                image: "图片"
            },
            typeLabels: {
                info: "资料",
                news: "新闻",
                image: "图片"
            }
        },
        style: {
            selectStyle: "改写风格",
            customRequirement: "自定义要求",
            customPlaceholder: "请输入自定义风格要求...",
            styles: {
                Professional: "更专业",
                Concise: "更简短",
                Friendly: "更友好",
                Colloquial: "更口语化",
                Assertive: "更强势",
                Restrained: "更克制",
                Custom: "自定义"
            },
            descriptions: {
                Professional: "使用专业术语和严谨表达",
                Concise: "精简内容，突出重点",
                Friendly: "亲切自然的语气",
                Colloquial: "接近日常说话的方式",
                Assertive: "自信有力的表达",
                Restrained: "含蓄内敛的表达",
                Custom: "输入自定义风格要求"
            }
        },
        struct: {
            selectStructure: "结构优化",
            before: "修改前",
            after: "修改后",
            structures: {
                "De-Redundancy": "去冗余表达",
                "Information-Layering": "信息分层",
                "Point-Form": "要点化",
                "Short-Sentencing": "短句化",
                "Data-Highlighting": "关键数据凸显"
            },
            descriptions: {
                "De-Redundancy": "删除冗余重复的表达",
                "Information-Layering": "按重要性层次组织信息",
                "Point-Form": "转换为要点列表形式",
                "Short-Sentencing": "拆分为简短易读的句子",
                "Data-Highlighting": "突出关键数据和事实"
            },
            examples: {
                "De-Redundancy": {
                    before: "我个人认为，这个方案目前来说是比较可行的，并且我觉得可以尽快推进。",
                    after: "这个方案**可行**，可以**尽快推进**。"
                },
                "Information-Layering": {
                    before: "新版本上线后注册量增长 28%，付费转化率提升 6%，增长主要来自移动端用户。",
                    after: "### 核心结果\n\n注册量增长 **28%**，付费转化率提升 **6%**。\n\n> 主要来源：移动端用户"
                },
                "Point-Form": {
                    before: "本周需要完成需求确认、视觉设计和开发排期，并在周五前同步风险。",
                    after: "- 确认需求\n- 完成视觉设计\n- 排定开发时间\n- 周五前同步风险"
                },
                "Short-Sentencing": {
                    before: "因为需求变更较多且交付时间提前，所以团队需要重新评估范围并优先完成核心功能。",
                    after: "需求发生多次变更。  \n交付时间也提前了。  \n团队将重新评估范围。  \n**优先完成核心功能。**"
                },
                "Data-Highlighting": {
                    before: "本月新增用户 1280 人，较上月增长 23%，其中付费用户 186 人。",
                    after: "| 指标 | 结果 |\n| --- | ---: |\n| 新增用户 | **1,280 人** |\n| 环比增长 | **23%** |\n| 付费用户 | **186 人** |"
                }
            }
        },
        toast: {
            customStyleRequired: "需要自定义风格",
            customStyleRequiredDesc: "请输入自定义风格要求",
            retryError: "改写失败，请重试",
            contentApplied: "内容已应用",
            materialRequired: "需要选择素材",
            materialRequiredDesc: "请至少选择一个素材",
            fetchMaterialsFailed: "获取素材列表失败",
            generateSuccess: "生成成功",
            generateFailed: "生成失败",
            submitted: "AI 改写任务已提交",
            submittedDesc: "可在任务面板中查看处理进度。",
            resultReady: "可在任务面板中打开并查看改写结果",
            timeout: "AI 改写超时",
            timeoutDesc: "AI 改写任务超时，请重试",
            taskExpired: "AI 改写任务已过期",
            quotaExceeded: "存储空间不足，已自动清理旧任务",
            applySourceChanged: "无法应用改写结果，原始文本已变化或找不到。",
            missingSourceText: "找不到原始文本，无法应用改写结果。",
        },
    },
    aiMindmap: {
        designLoading: "正在加载导图配色",
        designLoadFailed: "导图配色加载失败",
        retryDesign: "重试",
        title: "思维导图",
        description: "打开历史导图，或基于全文/选区重建；支持拖拽、双击编辑与快捷键操作",
        loading: "正在生成思维导图...",
        empty: "暂无可展示的思维导图",
        revision: "版本",
        source: {
            history: "历史导图",
            fullArticle: "全文生成",
        },
        status: {
            synced: "已同步",
            dirty: "待保存",
        },
        actions: {
            regenerateFull: "按全文重建",
            save: "保存导图",
            saveDirty: "保存修改",
            download: "下载 XMind",
            center: "居中",
            fitView: "适应视图",
        },
        canvas: {
            article: "文章",
            hint: "双击编辑，Tab 新增子节点，Enter 新增同级节点，拖拽可调整结构",
        },
        side: {
            left: "左侧分支",
            right: "右侧分支",
        },
        nodeEditor: {
            current: "当前节点",
            text: "节点内容",
            textPlaceholder: "请输入节点文本",
            note: "节点备注",
            notePlaceholder: "补充该节点的说明、上下文或执行备注",
            addChild: "新增子节点",
            addSibling: "新增同级节点",
            editInline: "画布内编辑",
            focus: "聚焦节点",
            side: "分支方向",
            children: "子节点数",
            expand: "展开节点",
            collapse: "折叠节点",
            delete: "删除节点",
            newChild: "新子节点",
            newSibling: "新同级节点",
            untitled: "未命名节点",
            hint: "根节点不可删除。节点文本与备注支持在右侧修改，也支持直接在画布中双击编辑。",
        },
        toast: {
            articleRequired: "请先保存文章",
            articleRequiredDesc: "仅支持在文章编辑模式下生成思维导图",
            emptyArticle: "文章内容为空",
            emptyArticleDesc: "请先输入文章内容或选中文字",
            generated: "思维导图已生成",
            generateFailed: "生成思维导图失败",
            loadFailed: "加载思维导图失败",
            saveSuccess: "思维导图已保存",
            saveFailed: "保存思维导图失败",
            downloadSuccess: "XMind 文件已下载",
            downloadFailed: "下载 XMind 文件失败",
            revisionConflict: "导图版本冲突",
            revisionConflictDesc: "当前导图已被更新，请重新加载后再保存",
            retryError: "操作失败，请重试",
        },
    },
    cookieBanner: {
        types: {
            necessary: {
                name: "必要 Cookie",
                description: "这些 cookie 对于网站正常运行是必需的,无法关闭。它们有助于登录和设置隐私偏好等操作。"
            },
            analytics: {
                name: "分析",
                description: "这些 Cookie 通过跟踪哪些页面最受欢迎以及访问者如何在网站上浏览来帮助我们改进网站。"
            }
        },
        banner: {
            description: "我们在网站上使用 Cookie 来提升您的用户体验、提供个性化内容并分析网站流量。<a href=\"/cookie-policy\" target=\"_blank\">Cookie 政策</a>",
            acceptAll: "全部接受",
            acceptAllLabel: "接受所有 Cookie",
            rejectNonEssential: "拒绝非必要 Cookie",
            rejectNonEssentialLabel: "拒绝非必要 Cookie",
            preferences: "偏好设置",
            preferencesLabel: "切换偏好设置"
        },
        preferences: {
            title: "自定义您的 Cookie 偏好设置",
            description: "我们尊重您的隐私权。您可以选择不允许某些类型的 Cookie。您的 Cookie 偏好设置将应用于我们整个网站。",
            creditLink: "免费获取此横幅",
            creditLinkLabel: "免费获取此横幅"
        }
    },
    cookiePolicy: {
        title: "Cookie 政策",
        subtitle: "了解我们如何使用 Cookie 和类似技术",
        introduction: "本 Cookie 政策解释了 JoyfulWords（创作者工具箱）如何使用 Cookie 和类似技术，以及您如何管理您的 Cookie 偏好设置。",
        whatAreCookies: {
            title: "什么是 Cookie？",
            description: "Cookie 是当您访问网站时存储在您的设备上的小型文本文件。它们被广泛用于记住您的偏好、让您在网站之间保持登录状态、了解您如何使用网站以及提供相关内容。"
        },
        types: {
            title: "我们使用的 Cookie 类型",
            necessary: {
                name: "必要 Cookie",
                description: "这些 Cookie 对于网站的正常运行是必需的，没有它们，网站无法正常工作。它们通常仅在您执行特定操作时才设置。",
                examples: [
                    "保持您的登录状态",
                    "记住您的隐私偏好设置",
                    "确保网站安全运行"
                ]
            },
            analytics: {
                name: "分析 Cookie",
                description: "这些 Cookie 帮助我们了解访客如何使用我们的网站，收集哪些页面最受欢迎以及访客如何浏览网站等信息。这些信息帮助我们改进网站的性能和用户体验。",
                examples: [
                    "跟踪页面访问量和用户行为",
                    "分析网站性能和用户参与度",
                    "识别网站问题和改进机会"
                ]
            }
        },
        howToManage: {
            title: "如何管理 Cookie",
            description: "您可以通过多种方式管理 Cookie。您可以通过网站的 Cookie 横幅设置您的偏好，也可以通过浏览器设置管理所有 Cookie。",
            browserSettings: {
                title: "浏览器设置",
                chrome: "Chrome: 设置 > 隐私和安全 > Cookie 和其他网站数据",
                firefox: "Firefox: 选项 > 隐私与安全 > Cookie 和网站数据",
                safari: "Safari: 偏好设置 > 隐私 > 管理 Cookie",
                edge: "Edge: 设置 > Cookie 和网站权限 > Cookie 和网站数据"
            }
        },
        updates: {
            title: "政策的更新",
            description: "我们可能会不时更新本 Cookie 政策。任何变更都将在此页面上发布，并会在政策生效前通过网站通知您。"
        },
        contact: {
            title: "联系我们",
            description: "如果您对本 Cookie 政策有任何疑问或疑虑，请通过以下方式联系我们：",
            email: "support@joyword.link"
        },
        lastUpdated: "最后更新"
    },
    legal: {
        termsOfUse: "使用条款",
        privacyPolicy: "隐私政策",
        cookiePolicy: "Cookie 政策"
    },
    termsOfUse: {
        title: "使用条款",
        subtitle: "欢迎使用 JoyfulWords（创作者工具箱）",
        introduction: "欢迎使用 JoyfulWords（创作者工具箱）。这些使用条款 govern 您使用我们的网站和服务。通过访问或使用我们的服务，您同意受这些条款的约束。",
        acceptance: {
            title: "接受条款",
            description: "通过创建账户或使用我们的服务，您确认您已阅读、理解并同意受这些条款的约束。如果您不同意这些条款，请不要使用我们的服务。"
        },
        services: {
            title: "我们的服务",
            description: "JoyfulWords 是一个内容创作工具平台，提供以下功能：",
            features: [
                "网络素材搜索和管理",
                "AI 辅助文章创作",
                "AI 一键生图",
                "AI 一键 PPT"
            ]
        },
        responsibilities: {
            title: "用户责任",
            description: "使用我们的服务时，您同意：",
            items: [
                "提供准确、完整的注册信息",
                "维护账户安全性，并对账户下的所有活动负责",
                "遵守所有适用的法律法规",
                "不滥用服务或干扰其他用户体验",
                "不得使用服务生成、上传、发布或传播禁止内容，包括成人/NSFW、暴力或非法内容"
            ]
        },
        intellectualProperty: {
            title: "知识产权",
            description: "服务中的所有内容，包括但不限于文本、图形、logo、图像和软件，均为 JoyfulWords 或其内容供应商的财产，受知识产权法保护。未经书面许可，您不得复制、修改、分发或以其他方式使用我们的内容。"
        },
        liability: {
            title: "责任限制",
            description: "在法律允许的最大范围内，JoyfulWords 不对因使用或无法使用服务而导致的任何间接、偶然、特殊或后果性损害承担责任。"
        },
        termination: {
            title: "服务终止",
            description: "我们保留随时暂停或终止您访问服务的权利，无需提前通知，原因包括但不限于违反这些条款。"
        },
        changes: {
            title: "条款变更",
            description: "我们可能会不时更新这些条款。变更后继续使用服务即表示您接受更新后的条款。重大变更将通过网站通知您。"
        },
        governingLaw: {
            title: "适用法律",
            description: "这些条款受您所在司法管辖区的法律管辖。"
        },
        contact: {
            title: "联系我们",
            description: "如果您对这些使用条款有任何疑问，请通过以下方式联系我们：",
            email: "support@joyword.link"
        },
        lastUpdated: "最后更新"
    },
    privacyPolicy: {
        title: "隐私政策",
        subtitle: "了解我们如何保护您的信息",
        introduction: "JoyfulWords（创作者工具箱）致力于保护您的隐私。本隐私政策解释了我们如何收集、使用、披露和保护您的信息，符合 GDPR、CCPA 等国际隐私法规。",
        dataCollection: {
            title: "我们收集的信息",
            description: "我们收集以下类型的信息：",
            personalInfo: {
                title: "个人信息",
                items: [
                    "账户信息：电子邮件地址、姓名",
                    "使用数据：创建的文章、素材、设置",
                    "支付信息：支付交易详情（通过第三方支付处理器处理）"
                ]
            },
            usageData: {
                title: "使用数据",
                items: [
                    "访问日志：IP 地址、浏览器类型、设备信息",
                    "使用统计：页面访问、功能使用情况",
                    "Cookie 数据：通过 Cookie 收集的偏好设置"
                ]
            }
        },
        dataUsage: {
            title: "我们如何使用您的信息",
            purposes: [
                "提供、维护和改进我们的服务",
                "处理交易和发送相关通知",
                "向您发送技术通知、更新和安全警报",
                "响应您的评论、问题和请求",
                "监控和分析使用趋势，以改进服务"
            ]
        },
        dataSharing: {
            title: "信息披露",
            description: "我们不会出售、出租或交易您的个人信息。我们仅在以下情况下披露信息：",
            exceptions: [
                "经您同意",
                "为遵守法律义务或响应法院命令",
                "保护我们的权利、财产或安全",
                "与可信服务提供商合作（在保密协议下）"
            ]
        },
        security: {
            title: "数据安全",
            description: "我们采取合理的技术和组织措施来保护您的信息免受未经授权的访问、使用或披露。然而，没有任何传输或存储方法是完全安全的。"
        },
        rights: {
            title: "您的权利（GDPR/CCPA）",
            description: "根据适用的隐私法律，您拥有以下权利：",
            items: [
                "访问权：请求访问我们持有的您的个人信息副本",
                "更正权：请求更正不准确的信息",
                "删除权：请求删除您的个人信息",
                "限制处理权：请求限制我们如何使用您的信息",
                "数据可携权：请求将您的信息传输到另一个服务",
                "反对权：反对我们处理您的信息"
            ]
        },
        retention: {
            title: "数据保留",
            description: "我们会在实现本政策所述目的所需的时间内保留您的个人信息，除非法律要求或允许更长的保留期限。"
        },
        international: {
            title: "国际数据传输",
            description: "您的信息可能会被传输到您所在国家/地区以外的国家/地区并在那里处理。我们将采取适当措施确保您的数据得到保护。"
        },
        children: {
            title: "儿童隐私",
            description: "我们的服务不面向 16 岁以下的儿童。我们不会故意收集 16 岁以下儿童的个人信息。如果我们发现我们已收集此类信息，我们将采取步骤删除它。"
        },
        cookies: {
            title: "Cookies 和跟踪技术",
            description: "我们使用 Cookie 和类似技术来收集和跟踪信息。您可以通过浏览器设置管理 Cookie 偏好。有关详细信息，请参阅我们的",
            cookiePolicyLink: "Cookie 政策"
        },
        changes: {
            title: "政策变更",
            description: "我们可能会不时更新本隐私政策。变更后继续使用服务即表示您接受更新后的政策。重大变更将通过网站通知您。"
        },
        contact: {
            title: "联系我们",
            description: "如果您对本隐私政策有任何疑问或想要行使您的权利，请通过以下方式联系我们：",
            email: "support@joyword.link"
        },
        lastUpdated: "最后更新"
    },
    landing: {
        nav: {
            menu: "菜单",
            menuDescription: "移动端主导航菜单",
            features: "功能",
            pricing: "价格",
            blog: "博客",
            mcp: "MCP",
            tools: "工具箱",
            fileConverter: "文件转换",
            myArticles: "我的文章",
            startCreating: "开始创作 →",
        },
        badge: "AI 内容创作工作台",
        heading: "一个想法，生成",
        headingAccent: "文章、信息图、",
        headingAccentSuffix: "PPT 和思维导图",
        description: "JoyfulWords 是 AI 内容创作工作台：快速找寻素材、生成成稿，再一键把文章转换成信息图、演示 PPT 和思维导图，一个工作流，四种产出。",
        cta: "免费开始创作 · 注册送 200 积分",
        watchDemo: "看 60 秒演示",
        stats: {
            subscription: {
                value: "0",
                label: "订阅费，按量付费",
            },
            workflow: {
                value: "4 + 1",
                label: "4 大生成能力，1 个工作流",
            },
            credit: {
                value: "按量计费",
                label: "积分随内容难度动态消耗",
            },
        },
        media: {
            heroLabel: "JoyfulWords 四种内容产出总览",
            heroPosterAlt: "JoyfulWords 内容工作流预览",
            heroCaption: "从一个选题出发，在同一工作流里生成四种内容。",
            exploreFeatures: "查看四大能力",
        },
        howItWorks: {
            eyebrow: "How it works",
            title: "从想法到发布，只需四步",
            description: "素材、大纲、成稿与多形态导出在同一个页面里完成，不用在工具之间来回粘贴。",
            steps: {
                research: {
                    title: "快速找寻素材",
                    description: "全网实时搜索，链接与灵感统一入库，几秒找到可用素材。",
                },
                outline: {
                    title: "梳理大纲",
                    description: "AI 提炼观点与结构，先把文章骨架立起来。",
                },
                draft: {
                    title: "生成成稿",
                    description: "提交 AI 帮写后异步生成完整草稿，再打开编辑器继续完善。",
                },
                export: {
                    title: "一键多态导出",
                    description: "文章、信息图、PPT、思维导图同步就绪。",
                },
            },
        },
        featuresHeading: "四大生成能力，",
        featuresHeadingAccent: "全部内置",
        featuresSubheading: "一份内容，连接四种形态。文章、信息图和思维导图可衔接使用，PPT 展示已实现的代码流程预览。",
        features: {
            article: {
                eyebrow: "AI Writing",
                title: "快速生成文章",
                description: "从一个选题出发，大纲、正文、配图一次生成结构完整的初稿，支持续写、改写、风格切换。",
                point1: "填写写作需求并选择文章风格",
                point2: "任务状态从初始化到草稿清晰可见",
                point3: "打开三栏编辑器继续改写与完善",
                cta: "试试 AI 写作",
                mediaLabel: "快速生成文章功能演示",
                posterAlt: "JoyfulWords AI 文章写作工作台",
            },
            infographic: {
                eyebrow: "Infographic",
                title: "生成信息图",
                description: "把文字要点一键转成可分享的信息图，长文拆解、数据表达、社媒分发都适用。",
                point1: "要点自动排版成图",
                point2: "多风格模板一键切换",
                point3: "方向、语言与内容密度均可配置",
                cta: "生成一张信息图",
                mediaLabel: "生成信息图功能演示",
                posterAlt: "JoyfulWords 专业信息图样式预览",
            },
            ppt: {
                eyebrow: "Presentation",
                title: "生成 PPT",
                description: "仓库中的完整 PPT 工作流已经实现；当前页面入口暂未开放，本演示按真实代码流程预览。",
                point1: "先生成可编辑故事卡并确认内容",
                point2: "再选择主题、图片风格与转场",
                point3: "完成后可预览并下载 PPT",
                cta: "暂未开放 · 流程预览",
                mediaLabel: "PPT 代码流程预览",
                posterAlt: "文章转换为 PPT 的功能预览",
            },
            mindmap: {
                eyebrow: "Mind Map",
                title: "生成思维导图",
                description: "复杂内容自动梳理成思维导图，结构一目了然，还能继续编辑并下载为 XMind。",
                point1: "长文自动提炼层级结构",
                point2: "分支节点可拖拽编辑",
                point3: "保存修改并下载 XMind 文件",
                cta: "生成思维导图",
                mediaLabel: "生成思维导图功能演示",
                previewLabel: "内容主题",
                previewBranchStructure: "文章结构",
                previewBranchIdeas: "关键观点",
                previewBranchVisuals: "视觉素材",
            },
        },
        outcomes: {
            eyebrow: "Outcomes",
            title: "把创作时间，",
            titleAccent: "还给创作本身",
            description: "两个效率承诺：找素材、创作不用切换平台；一篇文章直接转换成多种产物。",
            metrics: {
                platform: {
                    value: "1 个平台",
                    label: "找素材、写作、配图、导出，全程不切换页面",
                },
                copyPaste: {
                    value: "0 复制粘贴",
                    label: "素材搜到即插入文章，不再跨工具搬运",
                },
                outputs: {
                    value: "1 → 4",
                    label: "一篇文章直接转出信息图、PPT、思维导图",
                },
                search: {
                    value: "秒级",
                    label: "全网素材实时搜索，选题当场找到依据",
                },
            },
            pillarOne: {
                title: "找素材、创作，不用切换平台",
                before1: "以前，找素材开 3 个标签页，写作、做图各用一个工具，来回复制粘贴。",
                before2: "以前，素材存在收藏夹里，写的时候又找不到。",
                after1: "现在，全网搜索、素材入库、引用成稿，都在同一个编辑页面里完成。",
                after2: "现在，素材点一下就进文章，注意力不再被打断。",
                beforeLabel: "多平台来回切换",
                afterLabel: "一个页面完成创作",
                mediaLabel: "创作工作流改版前后对比",
            },
            pillarTwo: {
                title: "一篇文章，转换成多种产物",
                before1: "以前，同一个内容，做图、做 PPT、画导图要重做三遍。",
                after1: "现在，写完文章，信息图、PPT、思维导图一键转换生成。",
                after2: "现在，一次创作沉淀为素材，下一篇继续复用。",
                source: "你的文章",
                outputInfographic: "信息图",
                outputPpt: "演示 PPT",
                outputMindmap: "思维导图",
            },
            scenes: {
                creator: {
                    title: "自媒体作者",
                    description: "一天产出 2 篇图文并茂的公众号或博客文章，配图和信息图同步完成。",
                },
                globalTeam: {
                    title: "出海内容团队",
                    description: "中英双语内容一次完成，SEO 关键词与结构化排版内置在工作流里。",
                },
                workReport: {
                    title: "职场汇报",
                    description: "把周报、方案文档一键转成 PPT 和思维导图，几分钟完成一次演示。",
                },
            },
        },
        faq: {
            title: "常见问题",
            items: {
                free: {
                    question: "JoyfulWords 是免费的吗？",
                    answer: "注册即送 200 积分，可以免费体验写作、信息图、PPT 和思维导图。之后按量付费，积分消耗随内容难度动态计算，没有订阅费，用多少花多少。",
                },
                chatgpt: {
                    question: "和直接用 ChatGPT 写有什么区别？",
                    answer: "JoyfulWords 把素材搜索、素材库、文章编辑、配图、信息图、PPT 和思维导图放进同一个工作流。你不只得到一段回答，还能持续编辑、复用素材并完成多形态导出。",
                },
                exports: {
                    question: "支持哪些导出格式？",
                    answer: "文章可继续编辑与发布，信息图和思维导图可导出为图片，演示内容可生成 PPT。具体可用格式以对应功能页面为准。",
                },
                copyright: {
                    question: "生成的内容版权归谁？",
                    answer: "你可以将生成结果用于自己的创作与发布。具体权利仍受输入素材、第三方许可与适用法律约束，请确保你有权使用上传的素材，并参考服务条款。",
                },
                collaboration: {
                    question: "支持团队协作吗？",
                    answer: "当前版本以个人创作工作流为主，团队协作能力正在规划中。你可以先通过导出文章、图片和 PPT 与团队共享成果。",
                },
            },
        },
        finalCta: {
            title: "你的下一篇内容，从这里开始",
            description: "免费注册，200 积分立刻到账。",
            cta: "免费开始创作",
        },
        footer: {
            description: "AI 内容创作工作台。一个想法，生成文章、信息图、PPT 和思维导图。",
            copyright: "© 2026 JoyfulWords，内容创作与管理工具",
            privacyPolicy: "隐私政策",
            termsOfUse: "服务条款",
            product: {
                title: "产品",
                create: "开始创作",
                pricing: "价格",
                mcp: "MCP",
            },
            resources: {
                title: "资源",
                blog: "博客",
                tools: "免费工具",
                fileConverter: "文件转换",
            },
            legal: {
                title: "法律",
                privacy: "隐私政策",
                terms: "服务条款",
                cookies: "Cookie 政策",
            },
        },
    },
    pricing: {
        nav: {
            backHome: "返回首页",
        },
        eyebrow: "无需订阅，随用随充",
        heading: "Pay as you go",
        description: "只在真正使用 AI 写作、生图、搜索或 PPT 生成时消耗积分。没有月费，没有套餐锁定，充值后按实际用量扣除。",
        exchangeRate: "小提示：1 积分 = 1 美分。",
        cta: {
            recharge: "充值积分",
            viewRates: "查看明细",
        },
        visual: {
            label: "Cost Estimate",
            title: "预估消耗",
            estimates: {
                subscription: {
                    value: "0",
                    label: "订阅费",
                    description: "随时充值，不绑套餐。",
                },
                article: {
                    value: "20-80 积分",
                    label: "写一篇优质文章预计",
                    description: "视长度、素材量和改写轮次浮动。",
                },
                ppt: {
                    value: "1 积分/张",
                    label: "生成 PPT 仅",
                    description: "按生成页数计费，每张 1 积分。",
                },
                image: {
                    value: "1-8 积分",
                    label: "图片生成",
                    description: "主流模型单张生成区间。",
                },
            },
        },
        detailLabel: "价格明细",
        detailHeading: "每一类消耗都被准确记录",
        table: {
            module: "模块",
            item: "子计费项",
            price: "价格",
        },
        billingGuide: {
            label: "功能计费说明",
            title: "按功能类型理解积分消耗",
            description: "不同功能会消耗不同底层资源。实际扣费以使用记录为准，免费功能不会扣除积分。",
            table: {
                category: "分类",
                billing: "计费说明",
                features: "适用功能",
            },
            categories: {
                llm: {
                    title: "重度使用 LLM",
                    billing: "按 LLM 使用量收费",
                    items: ["AI 编辑", "AI 写作", "搜索白板", "AI 图表"],
                },
                image: {
                    title: "轻度使用 LLM",
                    billing: "只对图片收费",
                    items: ["AI 封面-字体设计", "AI 信息图"],
                },
                audio: {
                    title: "轻度使用 LLM",
                    billing: "只对音频收费",
                    items: ["转播客音频"],
                },
                search: {
                    title: "轻度使用 LLM",
                    billing: "只对搜索收费",
                    items: ["资料搜索", "新闻搜索", "图片搜索"],
                },
                free: {
                    title: "永久免费",
                    billing: "不消耗积分",
                    items: ["编辑器", "素材库", "思维导图", "转换 Word 格式", "转换微信公众号格式"],
                },
            },
        },
        units: {
            credits: "积分",
            perKInput: "积分/k token input",
            perKOutput: "积分/k token output",
            perImage: "积分/张",
            perSlide: "积分/张",
            perAudioClip: "积分/段音频",
            perSearch: "积分/次",
            perGeneration: "积分/次",
        },
        sections: {
            ai: {
                eyebrow: "文字模型",
                title: "使用 LLM 模型",
                description: "输入和输出分开计费，适合长文、改写、大纲、摘要等不同长度任务。",
                standard: {
                    name: "普通模型",
                    description: "适合日常写作、整理、摘要和轻量改写。",
                    input: "普通模型 · input",
                    output: "普通模型 · output",
                },
                premium: {
                    name: "高级模型",
                    description: "适合复杂推理、长上下文、精修和高质量生成。",
                    input: "高级模型 · input",
                    output: "高级模型 · output",
                },
            },
            image: {
                gpt25Spec: "1K · 中等质量 · 有无参考图同价",
                eyebrow: "视觉生成",
                title: "AI 生图",
                description: "不同模型按单张图片扣费，便于在质量、速度和成本之间选择。",
            },
            layer: {
                title: "图层拆分",
                description: "对单张图片做图层拆分，适合后续编辑、重组和设计复用。",
            },
            materials: {
                eyebrow: "素材能力",
                title: "素材搜索",
                description: "搜索按次数扣费，PPT 生成按页扣费，适合边找资料边完成内容交付。",
            },
            search: {
                info: "Info 搜索",
                news: "News 搜索",
                image: "Image 搜索",
            },
            ppt: {
                title: "PPT 生成",
                item: "PPT 生成",
                description: "基于内容生成演示结构，按生成页数扣费。",
            },
            audio: {
                title: "音频生成",
                item: "音频生成",
                description: "按生成的音频段数扣费。",
            },
        },
        finalCta: {
            label: "开始使用",
            title: "先充值，再按实际创作消耗",
            description: "价格页只展示消耗规则。进入工作台后可以在计费管理中充值、查看余额和使用记录。",
            button: "前往充值",
        },
    },
    mcpPage: {
        nav: {
            backHome: "返回官网",
        },
        eyebrow: "标准 HTTP MCP Server",
        heading: "Connect to AI Agent",
        description: "支持标准的 MCP Server，支持 Claude Code 或任意 AI Agent。",
        agentHint: "如果不会用，直接复制命令让 AI Agent 自己安装即可。",
        command: {
            label: "Claude Code 安装命令",
            transport: "HTTP Transport",
            copy: "复制命令",
            copied: "已复制",
        },
        stepsLabel: "配置流程",
        stepsHeading: "Setup Flow",
        steps: {
            add: {
                title: "添加 MCP 服务",
                description: "运行上方命令，把 joyfulwords 作为标准 HTTP MCP Server 添加到 Claude Code。",
            },
            launch: {
                title: "启动 Claude Code",
                description: "重新启动 Claude Code，让本地客户端加载刚刚新增的 MCP 服务配置。",
            },
            verify: {
                title: "执行 /mcp",
                description: "在 Claude Code 中执行 /mcp，检查 joyfulwords 服务并启用可用能力。",
            },
        },
    },
    toolsPage: {
        nav: {
            workspace: "创作工作台",
            login: "登录",
        },
        eyebrow: "JoyfulWords 工具箱",
        title: "工具箱",
        subtitle: {
            intro: "选择一个工具，直接开始转换、生成或可视化。已开放功能支持游客体验。",
            workspacePrompt: "需要管理文章和任务？",
            workspaceLink: "进入创作工作台",
        },
        acquisition: {
            eyebrow: "JoyfulWords 创作者工具箱",
            headlinePrimary: "把内容，变成",
            headlineAccent: "可交付的作品",
            description: "转换文档、生成 PPT、创作配图和整理数据，开放工具无需登录即可开始。",
            primaryAction: "免费转换为 Word",
            primaryHint: "游客可用 · 直接下载 DOCX",
            secondaryAction: "查看全部 6 个工具",
            heroAlt: "纸张内容转换为演示文稿、图片和图表的创作场景",
            demoLabel: "内容到成品的完整演示",
            demoCaption: "看看一份内容如何变成 PPT、信息图和可交付文档",
            demoDuration: "点击播放 · 60 秒内了解",
            benefitsLabel: "工具箱使用优势",
            benefits: {
                guest: {
                    title: "游客直接使用",
                    description: "开放工具无需注册",
                },
                editable: {
                    title: "结果可继续编辑",
                    description: "支持 Word、PPTX 和图片",
                },
                workflow: {
                    title: "一个入口完成",
                    description: "从内容到交付物",
                },
            },
        },
        showcase: {
            eyebrow: "真实功能演示",
            title: "先看效果，再选择工具",
            description: "每段演示都对应一个真实入口，看清工作流和输出结果后直接开始。",
            visualAlt: "纸张内容展开为多张视觉作品的创作场景",
            dataAlt: "纸张数据转换为清晰图表的创作场景",
        },
        toolActions: {
            "ai-writer": "开始写作",
            "image-generator": "免费创作图片",
            infographic: "生成信息图",
            "mind-map": "生成思维导图",
            "ai-charts": "生成 AI 图表",
            "ppt-generator": "开始生成 PPT",
            "markdown-to-word": "免费转为 Word",
            "ppt-to-word": "提取为 Word",
            "word-to-ppt": "生成 PPT",
            "meme-inserter": "查找表情包",
        },
        directory: {
            title: "选择任务，直接开始",
            description: "转换文档、生成图片和演示，或把数据整理成图表。每个工具都可独立使用。",
        },
        status: "待实现",
        availableStatus: "可使用",
        openPlaceholder: "查看工具页",
        metrics: {
            label: "工具箱概览",
            tools: {
                value: "6",
                label: "可用工具",
            },
        },
        sections: {
            gridLabel: "工具列表",
        },
        categories: {
            visual: {
                title: "视觉生成",
                description: "配图和信息图集中处理。",
            },
            data: {
                title: "数据可视化",
                description: "把数据和观点转换成可读图表。",
            },
            documents: {
                title: "文档转换",
                description: "围绕 Word、PPT 和 Markdown 的文件流转。",
            },
            writing: {
                title: "写作与结构",
                description: "从想法、草稿到结构整理。",
            },
        },
        tools: {
            "ai-writer": {
                title: "AI 写作",
                description: "随便写点，先把想法变成可以继续整理的文字。",
                category: "文字生产",
                meta: "随便写点",
            },
            "image-generator": {
                title: "创作图片",
                description: "输入画面描述，生成封面、文章配图和营销素材。游客每天可试用 3 张。",
                category: "视觉资产",
                meta: "Create Image",
            },
            infographic: {
                title: "信息图",
                description: "粘贴观点、步骤或数据，自动整理为适合分享的信息图。",
                category: "内容可视化",
                meta: "知识卡片",
            },
            "mind-map": {
                title: "思维导图",
                description: "把复杂主题拆成层级结构，用来做选题拆解、课程大纲和文章规划。",
                category: "结构整理",
                meta: "大纲规划",
            },
            "ai-charts": {
                title: "AI 图表",
                description: "输入数据、观点或分析结果，生成适合报告与复盘的可读图表。",
                category: "数据表达",
                meta: "图表生成",
            },
            "ppt-generator": {
                title: "PPT 生成",
                description: "粘贴 Markdown 或选择文章，确认内容结构与模板后导出可编辑 PPTX。",
                category: "演示输出",
                meta: "文章到 PPTX",
            },
            "markdown-to-word": {
                title: "Markdown 转 Word",
                description: "粘贴 Markdown，选择文档模板，下载可继续编辑的 DOCX。游客可直接使用。",
                category: "文档转换",
                meta: "MD 到 DOCX",
            },
            "ppt-to-word": {
                title: "PPT 转 Word",
                description: "上传 PPTX，提取标题、正文和演讲者备注，整理为 Word 文档。",
                category: "文档转换",
                meta: "PPTX 到 DOCX",
            },
            "word-to-ppt": {
                title: "Word 转 PPT",
                description: "把文档内容整理为演示结构，快速生成可继续加工的幻灯片大纲。",
                category: "文档转换",
                meta: "DOCX 到 PPTX",
            },
            "meme-inserter": {
                title: "智能插入表情包",
                description: "根据段落语气和上下文，推荐适合插入文章或社媒内容的表情包。",
                category: "视觉资产",
                meta: "情绪增强",
            },
        },
        activities: {
            title: "活动菜单",
            cta: "活动功能待实现",
            checkIn: {
                title: "每日签到",
                reward: "+ 积分",
                description: "为轻量使用预留的积分入口。",
            },
            share: {
                title: "分享得积分",
                reward: "邀请",
                description: "后续可承接邀请、推广和分享奖励。",
            },
            campaign: {
                title: "限时活动",
                reward: "活动",
                description: "承载节日活动、功能试用和限时任务。",
            },
        },
        imageGenerator: {
            authChecking: "检查登录状态",
            guestTrial: "游客试用：每日 3 张",
            signedIn: "已登录：使用账户额度",
            balanceLoading: "正在刷新积分",
            signedInBalance: "账户积分：{credits}",
        },
        infographic: {
            signedIn: "已登录：使用账户额度",
            loginRequiredTitle: "请登录后操作",
            loginRequiredDescription: "生成信息图需要登录账户。你可以先填写内容和参数，登录后再提交生成。",
            sourceTextLabel: "信息图内容",
            sourceTextPlaceholder: "书写或复制一段文字，自动分析主题并生成信息图。",
            sourceTextHint: "建议输入观点、步骤、清单、数据说明或文章摘要，系统会自动提炼主题并组织成信息图。",
            toast: {
                enterTextFirst: "请先输入内容",
                enterTextFirstDesc: "信息图需要基于一段文字生成。",
            },
        },
        aiCharts: {
            guestTrial: "游客试用：每日 20 次",
            signedIn: "已登录：使用账户额度",
            timelineLabel: "AI 图表生成步骤",
            steps: {
                data: {
                    title: "输入数据",
                    description: "粘贴表格、清单或分析文本",
                },
                requirement: {
                    title: "描述图表",
                    description: "选择表达方式和基础样式",
                },
                preview: {
                    title: "查看图表",
                    description: "微调样式并下载 PNG",
                },
            },
            dataTextLabel: "数据文本",
            dataTextPlaceholder: "粘贴数据、结论、表格或一段包含数字的分析文本。",
            requirementLabel: "生成要求",
            requirementPlaceholder: "例如：用柱状图展示季度收入变化，突出 Q2 增长。",
            charCount: "{count} 字",
            useExample: "使用示例",
            generate: "生成图表",
            previewTitle: "图表预览",
            previewEmpty: "等待生成",
            emptyPreview: "输入数据和生成要求后，图表会显示在这里。",
            generatingPreview: "正在生成图表...",
            generationFailed: "图表生成失败，请稍后重试。",
            generationTimeout: "图表生成等待超时，请稍后重试。",
            guestResult: "游客结果：仅本页预览与下载",
            accountResult: "账户图表 #{id}",
            displaySettings: "展示设置",
            downloadPng: "下载 PNG",
            example: {
                data: "Q1 12800, Q2 16400, Q3 14200, Q4 19800",
                requirement: "用柱状图展示季度收入变化，突出 Q4 的增长。",
            },
            toast: {
                enterInputTitle: "请补充数据和要求",
                enterInputDescription: "AI 图表需要数据文本和生成要求。",
                generated: "图表已生成",
                downloaded: "PNG 已下载",
                downloadFailed: "PNG 下载失败",
            },
        },
        pptGenerator: {
            title: "PPT 生成",
            description: "粘贴 Markdown 或选择已有文章，先确认内容结构，再进入 Storycard 与模板流程。",
            authLoading: "正在检查登录状态",
            loginTitle: "登录后生成 PPT",
            loginDescription: "PPT 生成依赖您的账户额度。登录后可粘贴 Markdown 或选择文章，并继续原有生成流程。",
            loginAction: "登录并继续",
            workflowLabel: "PPT 生成流程",
            workflowInput: "Markdown",
            workflowReview: "结构确认",
            workflowExport: "PPTX",
            loadingArticles: "正在加载文章",
            loadError: "文章加载失败，请稍后重试。",
            retry: "重试",
            emptyTitle: "暂无可用于生成 PPT 的文章",
            emptyDescription: "先创建一篇包含正文的文章，再返回这里生成 PPT。",
            createArticle: "前往文章工作台",
            sourceLabel: "内容来源",
            sourceDescription: "直接输入 Markdown，或复用文章工作台中的内容。",
            sourcePaste: "粘贴 Markdown",
            sourceArticle: "选择已有文章",
            pasteTitleLabel: "文章标题（可选）",
            pasteTitlePlaceholder: "未填写时将使用正文首行",
            pasteContentLabel: "Markdown 正文",
            pasteContentPlaceholder: "# 演示标题\n\n在这里粘贴 Markdown 正文，支持列表、表格、引用和代码块。",
            workspaceLabel: "Markdown 工作区",
            charCount: "{count} 字符",
            viewLabel: "Markdown 显示方式",
            viewEdit: "编辑",
            viewPreview: "预览",
            viewSplit: "分栏",
            editorPane: "Markdown 源码",
            previewPane: "渲染预览",
            previewEmptyTitle: "预览将在这里显示",
            previewEmptyDescription: "输入 Markdown 后，标题、列表、表格和引用会实时渲染。",
            markdownHint: "支持 GFM 格式：标题、列表、任务清单、表格、引用、代码块和链接。",
            pasteHint: "生成前会把 Markdown 转换为文章内容并保存，然后进入 Storycard 和 PPT 生成流程。",
            pasteContentRequired: "请先输入需要生成 PPT 的 Markdown 内容。",
            prepareError: "Markdown 内容保存失败，请稍后重试。",
            preparing: "正在准备内容",
            articleModeTitle: "从已有文章开始",
            articleModeDescription: "选择包含正文且未归档的文章，继续原有 Storycard 和 PPT 生成流程。",
            articleLabel: "选择文章",
            articlePlaceholder: "请选择一篇文章",
            articleHint: "只显示包含正文且未归档的文章。",
            untitledArticle: "未命名文章",
            start: "开始生成 PPT",
            viewProgress: "查看生成进度",
            progressTitle: "PPT 任务正在后台进行",
            progressDescription: "关闭弹窗不会中断任务，可随时返回查看当前步骤。",
        },
        tasks: {
            title: "任务列表",
            subtitle: "不绑定文章的全局任务记录。",
            loginTitle: "登录后查看任务",
            loginDescription: "图片、信息图、PPT 和文章任务会在这里同步。",
        },
        workspaceCta: {
            title: "写出更专业的文章",
            description: "进入工作台继续管理文章、素材和生成任务，接入您的完整创作流程。",
            action: "进入创作工作台",
        },
        detail: {
            back: "返回工具箱",
            pptTitle: "从文章生成 PPT",
            pptDescription: "PPT 生成需要从已保存的 JoyfulWords 文章开始。进入文章工作台，确认 Storycard 和模板后即可生成。",
            pptAction: "前往文章工作台",
            placeholderTitle: "工具正在排期",
            placeholderDescription: "当前页面先保留分享和 SEO 入口，正式能力接入后会在这里承载完整操作流。",
            disabledAction: "待实现",
            previewPrompt: "输入创作目标，生成可继续编辑的结果",
            previewSteps: {
                input: "输入",
                generate: "生成",
                export: "回流",
            },
            notesTitle: "设计预留",
            notes: {
                account: {
                    title: "账号上下文",
                    description: "保留登录态、积分和会员能力接入位置。",
                },
                tasks: {
                    title: "任务中心",
                    description: "图片、图表、PPT 等异步任务会在这里同步。",
                },
                activity: {
                    title: "活动激励",
                    description: "为签到、分享奖励和活动曝光预留展示区域。",
                },
            },
        },
    },
    blog: {
        common: {
            backToHome: "返回首页",
            backToBlog: "返回博客",
            readMore: "阅读全文",
            noPosts: "暂时还没有博客文章",
            missingArticle: "未找到对应文章",
            fallbackNotice: "当前语言版本暂未提供，已自动切换为 {locale} 内容。",
            localeLabel: "语言",
        },
        list: {
            title: "博客",
            subtitle: "分享产品更新、创作方法和实战经验。",
        },
        detail: {
            articleLabel: "文章",
        },
    },
    billing: {
        balance: {
            title: "积分余额",
            credits: "积分",
            refresh: "刷新",
            recharge: "充值",
            updatedAt: "更新于 {time}",
            cached: "缓存",
            exchangeRate: "100 credit = $1",
            fetchFailed: "获取余额失败",
            refreshFailed: "刷新余额失败",
            refreshSuccess: "刷新成功",
            rechargeComingSoon: "充值功能即将上线",
            rechargeComingSoonDesc: "我们正在完善充值功能，敬请期待",
        },
        insufficientCredits: {
            title: "积分余额不足",
            description: "您的积分余额不足以执行此操作",
            currentCredits: "当前积分",
            requiredCredits: "所需积分",
            shortageCredits: "缺少积分",
            recommendedRecharge: "建议充值",
            goToRecharge: "前往充值",
            cancel: "取消",
        },
        payment: {
            dialog: {
                title: "充值积分",
            },
            providers: {
                paypal: "PayPal",
                oxapay: "加密货币",
                stripe: "Stripe",
                creem: "Card & Wallets",
            },
            form: {
                selectTier: "选择充值档位",
                customAmount: "或输入自定义金额（最低 200 积分）",
                credits: {
                    label: "积分数",
                    hint: "100 积分 = $1 USD，必须是 100 的倍数",
                    required: "请输入积分数",
                    invalid: "积分数格式不正确",
                    min: "最小充值 200 积分",
                    max: "最大充值 100,000 积分",
                    multiple: "积分数必须是 100 的倍数",
                },
                network: {
                    label: "网络类型",
                    placeholder: "请选择网络类型",
                    required: "请选择网络类型",
                    trc20: "TRC20 (Tron)",
                    erc20: "ERC20 (Ethereum)",
                },
                totalAmount: "支付金额",
                submit: "前往支付",
                payWithPaypal: "使用 PayPal 支付",
                paypalSecure: "安全的 PayPal 支付",
                payWithStripe: "使用 Stripe 支付",
                stripeSecure: "Stripe 提供的安全支付",
                payWithCardWallets: "使用 Card & Wallets 支付",
                cardWalletsSecure: "安全托管支付",
                submitting: "处理中...",
            },
            processing: "正在处理订单...",
            checkoutWaiting: {
                stillCreating: "支付页面仍在准备中",
                cancelNotice: "你可以继续等待，或取消本次操作。",
                cancel: "取消",
            },
            createFailed: {
                eyebrow: "准备未完成",
                title: "暂时无法打开支付页面",
                description: "请稍后重试，或返回计费页面。",
                loadFailed: "暂时无法读取订单，请返回计费页面后重试。",
                loading: "正在加载...",
                credits: "充值积分",
                creditsValue: "{credits} 积分",
                retryNotice: "重试后会重新为你准备支付页面。",
                retry: "重新尝试",
                retrying: "正在重试...",
            },
            success: {
                title: "充值成功！",
                desc: "已充值 {credits} 积分",
                confirming: "确认支付中...",
                confirmingDesc: "正在确认支付状态，请稍候",
                processing: "支付处理中",
                processingDesc: "正在获取订单信息，请稍候",
                processingHint: "您的支付正在处理中，请稍后返回订单页查看积分是否增加",
                pending: "等待确认...",
                pendingDesc: "支付已完成，正在充值中...",
                retryCount: "确认次数: {count}",
                backToBilling: "返回计费页面",
            },
            failed: {
                title: "支付失败",
                desc: "支付未能完成，请重试",
                retry: "重新尝试",
            },
            timeout: {
                title: "确认超时",
                desc: "支付确认时间过长，请返回查看或重试",
            },
            cancel: {
                title: "支付已取消",
                desc: "您已取消支付，可以随时重新充值",
                goBack: "返回上一页",
            },
            orderNo: "订单号",
            provider: "支付方式",
            error: {
                unauthorized: "未授权，请重新登录",
                createFailed: "创建订单失败",
                queryFailed: "查询订单失败",
                unknown: "未知错误",
            },
        },
        tabs: {
            recharges: "积分充值",
            usage: "积分使用",
            invoices: "发票",
        },
        recharges: {
            fetchFailed: "获取充值记录失败",
        },
        usage: {
            fetchFailed: "获取使用记录失败",
        },
        invoices: {
            fetchFailed: "获取发票列表失败",
            detailFetchFailed: "获取发票详情失败",
            status: {
                draft: "草稿",
                finalized: "已定稿",
                voided: "已作废",
                failed: "失败",
                pending: "处理中",
            },
            paymentStatus: {
                succeeded: "已支付",
                pending: "待支付",
                failed: "支付失败",
            },
            table: {
                issuingDate: "签发日期",
                number: "发票编号",
                status: "发票状态",
                paymentStatus: "支付状态",
                credit: "积分抵扣",
                totalAmount: "总金额",
            },
            detail: {
                title: "发票详情",
                number: "发票编号",
                issuingDate: "签发日期",
                status: "发票状态",
                paymentStatus: "支付状态",
                feesAmount: "费用金额",
                prepaidAmount: "预付费扣减",
                totalAmount: "总计金额",
                feeItems: "计费项明细",
                itemName: "项目",
                units: "数量",
                unitPrice: "单价",
                amount: "金额",
                createdAt: "创建时间",
            },
        },
        status: {
            pending: "处理中",
            settled: "已结算",
            failed: "失败",
        },
        table: {
            date: "时间",
            transactionId: "交易ID",
            description: "描述",
            amount: "金额",
            credits: "积分",
            status: "状态",
            metadata: "元数据",
            loading: "加载中...",
            noData: "暂无数据",
            totalInfo: "共 {total} 条，第 {page} 页",
            pageInfo: "第 {page} / {totalPages} 页",
            prevPage: "上一页",
            nextPage: "下一页",
            filterStatus: "状态：",
            filterStatusAll: "全部状态",
            filterStartDate: "开始时间：",
            filterEndDate: "结束时间：",
            search: "搜索",
            perPage: "每页",
            items: "条",
        },
        transaction: {
            recharge: "充值",
            usage: "消费",
        },
    },
};
