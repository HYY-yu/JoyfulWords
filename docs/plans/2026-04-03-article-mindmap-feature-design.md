...
4. 弹窗初始化：
   ...
   - 若不存在历史导图：有选区则按选区生成，否则按全文生成。
5. 用户在导图画布内直接完成核心操作：
   - 双击节点编辑文本
   - 拖拽调整结构
   - Tab / Enter 快捷新增节点
   - 右键菜单与工具栏完成展开、聚焦、缩放等操作
6. 右侧检查器只补充结构信息与备注编辑，不再承担主要编辑入口。
...
- `POST /api/article/mindmap/generate`
  - 输入：`article_id`, `article_text`, `selected_text?`
  - 输出：`MindMapDocument`
...
- 历史导图优先加载，选区/全文重建改为显式操作，避免用户打开弹窗时无感覆盖已有导图。
