...
5. [删除任务](#5-删除任务)
6. [删除抓取结果](#6-删除抓取结果)
7. [获取CrawSnapshot日志列表](#7-获取crowsnapshot日志列表)
...
```bash
curl -X DELETE http://localhost:8080/social/results/456 \
  -H "Accept-Language: zh-CN" \
  -H "Authorization: Bearer YOUR_ACCESS_TOKEN"
```
...
**注意:**
- 只能删除自己抓取的帖子
- 如果帖子被文章引用，则不允许删除（需先删除文章关联）
- 删除操作为硬删除，数据将永久移除

---

## 7. 获取CrawSnapshot日志列表
