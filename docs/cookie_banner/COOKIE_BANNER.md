...
- **登录态行为**: 未登录用户保留左下角 Cookie Icon；登录后隐藏浮动 Cookie Icon，并在文章页右上角账号菜单中提供 Cookie 偏好设置入口。
...
登录后不关闭 Cookie Banner 功能,只隐藏左下角浮动 Cookie Icon。用户仍可通过 `/articles` 右上角账号菜单中的“Cookie 偏好设置”打开 Silktide 偏好设置弹窗,调整或撤回 analytics consent。
