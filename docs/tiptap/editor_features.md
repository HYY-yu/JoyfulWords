...
公众号导出自动沿用与图表相同的设计来源：优先文章绑定快照，没有绑定则使用全局设计偏好，未设置偏好时使用共享目录默认的简约商务 / 绿色。不提供风格、主题或配色选择，不修改文章绑定或全局偏好。

现有六种风格（简约商务、杂志叙事、手绘插画、萌系 Q 版、高级毛玻璃、极简光影）决定标题、引用、图片等排版；五个色系对应的具体 palette 完整读取快照，包括主色、主色上的文字色、背景、表面、正文、辅助文字和边框。商务与杂志表格采用直角网格；手绘采用不规则圆角、不同粗细线条与本机楷体/手写字体，正文保留清晰的无衬线字体。毛玻璃与光影使用渐变、透明面板和 backdrop-filter 模糊作为渐进增强；目标编辑器移除模糊效果时仍保留底色与边框层次，字体效果取决于阅读设备可用字体。

共享解析位于 `lib/design/article-design.ts`，图表和公众号共同使用。每次打开导出或收到设计更新事件时重新读取；加载失败显示重试并禁用复制，避免悄悄使用错误配色。预览和复制复用同一份内联 HTML。字号、图片说明、外链脚注和阅读时间选项保留。

回归测试：`node --import tsx --test lib/wechat-markdown-export.test.ts lib/echarts/chart-design-loader.test.ts lib/echarts/joy-chart-theme.test.ts`。浏览器预览验收不代替微信公众号后台实际粘贴验收。
