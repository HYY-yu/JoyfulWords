    ...
    <div className="jw-app-shell min-h-screen">
      <header className="jw-app-header sticky top-0 z-40 border-b backdrop-blur-xl">
        <div className="mx-auto flex h-16 max-w-[1500px] items-center justify-between px-4 sm:px-6">
          <Link href={buildLocalizedPath(locale)} className="rounded-xl transition-transform hover:-translate-y-0.5">
            <BrandLogo />
          </Link>
          <div className="flex items-center gap-2">
            <Button asChild variant="ghost" size="sm" className="jw-themed-link h-8 rounded-full">
              <Link href={buildLocalizedPath(locale, "/tools")}>工具箱</Link>
            </Button>
            <Button asChild size="sm" className="jw-primary-button h-8 rounded-full px-3">
              <Link href="/articles">创作工作台</Link>
            </Button>
          </div>
        </div>
      </header>
